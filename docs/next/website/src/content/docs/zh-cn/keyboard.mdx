...
Herdr 是鼠标原生的。你可以点击窗格、标签页、工作区和智能体,拖动分割边框,使用右键菜单,一个按键绑定都不用学。键盘操作是可选的一层,不是必需项。 浮层里的滚轮只滚动视口，不改动键盘选中；窗格、侧栏与浮层的滚轮步进统一取 `ui.mouse_scroll_lines`（默认 3 行）。
...
随时按 `prefix+?` 可以查看所有生效的绑定。在快捷键帮助中按 `/` 即可筛选操作和快捷键；使用 Backspace 编辑筛选内容，或按 `ctrl+u` 清除。前缀模式等待下一个按键期间，会弹出 which-key 面板列出可用的后续绑定（可用 `ui.which_key = false` 关闭）。侧边栏底部的菜单按钮会打开模糊匹配的命令面板，覆盖所有动作（包括机器连接、编辑与从 SSH 配置导入，命令片段的管理与运行，场景快照，以及输入广播管理），搜索框为空时优先显示最近使用的命令。

各弹层页面有自己的局部按键，显示在页面底栏：machines 页面（`prefix+m`）上按 `b` 打开广播目标集合；机器动作键 —— `e` 编辑、`R` 重命名、`r` 重连、`f` 端口转发、`o` 远程文件浏览器、`d` 启用/禁用、`x` 移除、`v` 处理失败、`c` 复制修复命令 —— 在列表和机器详情卡上行为一致。「已复制修复命令」这类一次性反馈会显示在机器列表与机器详情卡底部，几秒后自动消失。输入广播开启期间，mode bar 常驻 `⇄ 广播 ×N` 徽标，让你绝不会忽视输入正在扇出到其他机器。
...
Goto 选择器(`prefix+g`)就是会话导航。搜索框会按空白把查询切成词,只保留每个词都出现在该行里的结果,所以 `server web` 也能找到 `web server`,词序不再影响命中。搜索或状态筛选生效时,默认选中第一个自身命中的行,而不是只为容纳它才被带出来的工作区行,回车直接打开你搜的东西。Home 依旧回到列表首行。(`prefix+w` 是另一个键:它只把侧边栏切到工作区导航模式,没有搜索框。)

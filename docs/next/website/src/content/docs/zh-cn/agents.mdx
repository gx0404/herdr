...
智能体会话开始前等你做决定的启动提示也算阻塞界面,例如 Codex 的「Hooks need review」选择框和 Kimi Code CLI 的「Trust this folder?」提示。提示打开期间窗格为 `blocked`,`herdr agent start` 会报告智能体尚未就绪,而不是直接返回成功。
...
上文的启动提示规则与内置、远程清单分开存放。Herdr 把它们叠加到当前生效的内置或远程清单上,远程清单更新不会把它们冲掉;生效清单里已有同 id 的规则时以该规则为准。本地覆盖替换整份清单,只运行覆盖文件里的规则。

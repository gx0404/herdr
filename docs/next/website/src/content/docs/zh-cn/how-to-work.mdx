...
同一个持久的 Herdr 会话就会在手机终端中打开。屏幕小于停靠布局的最小尺寸时,Herdr 切换到每次只显示一个面板的紧凑视图,所以你可以在不离开 SSH 的情况下检查智能体、切换工作区、查看窗格。紧凑视图的标题栏列出各个面板:点面板名即可切换过去;面板名放不下时,进「调整布局」按 `Tab` 切换。最小尺寸见[鼠标 UI](/zh-cn/docs/concepts/#鼠标-ui)。移动端切换器与 `ui.mobile_width_threshold` 属于经典布局,只有服务器太旧、不支持停靠布局时 Herdr 才会用经典布局。
...
<div class="mobile-doc-shots">
  ...
  <figure>
    <img src="/assets/mobile-switch-menu-v2.jpeg" alt="手机上 Herdr 经典布局的移动端切换器" loading="lazy" />
    <figcaption>移动端切换器(经典布局,旧服务器)</figcaption>
  </figure>
</div>

...
侧边栏行设置仅影响展开的桌面侧边栏。折叠视图和移动视图仍使用紧凑布局。在按分组显示的 agent 树里，上方分组头已经显示了机器、工作区和标签页，agent 行会省略 `machine`、`workspace`、`tab` 这几个 token；因此只剩 `state_icon` 的那一行会并入下一行。所有行都不含 `state_text` 时，放得下的话树会在 agent 名称后补上状态文字。
...
按智能体覆盖声音设置时，可使用 `default`、`on` 或 `off`。键是五家官方智能体的标签:`pi`、`claude`、`codex`、`open_code` 和 `kimi`。指向[已退役智能体](/zh-cn/docs/integrations/#已退役的集成)的键会被忽略并给出 `unknown config key` 诊断,同段其余设置照常生效。

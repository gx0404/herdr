...
你也可以打开 Herdr 的主菜单并选择「重载配置」（英文界面为 `reload config`）。
...
`language` 决定 Herdr 自身 TUI 与 CLI 消息的语言，支持 `"zh-CN"`（默认）与 `"en"`。

```toml
language = "en"
```

环境变量 `HERDR_LANG` 可在单次运行时覆盖它。包含这一项在内的客户端设置支持热重载：`reload config`（或主菜单中的「重载配置」）即生效，不需要重启窗格。
...
使用停靠终端组时，每个可见标签页都有自己的视图尺寸。旧版单视图客户端仍保持以下行为：只有一个客户端连接时，所有标签页都会采用它的尺寸。连接多个客户端时，每个正在查看的标签页会采用最后聚焦、选择或操作它的客户端尺寸。当只剩一个客户端时，所有标签页会立即恢复为该客户端的尺寸。没有客户端连接时，现有窗格 PTY 会保留最后的尺寸，新的无客户端布局会使用配置的回退值。
...
窗格由 Herdr 自己的终端层渲染，因此它们对外宣称 Herdr 身份，而不是启动 server 的终端：`TERM_PROGRAM=herdr`、`TERM_PROGRAM_VERSION` 为 Herdr 版本号、`TERM=xterm-256color`、`COLORTERM=truecolor`。宿主终端的身份变量（`WEZTERM_PANE`、`WEZTERM_UNIX_SOCKET`、`WEZTERM_EXECUTABLE`、`WEZTERM_EXECUTABLE_DIR`、`WEZTERM_CONFIG_FILE`、`WEZTERM_CONFIG_DIR`、`KITTY_WINDOW_ID`、`WT_SESSION`）会被移除，避免窗格内程序把 Herdr 误判成宿主终端。`HERDR_ENV=1` 始终注入，用于标记运行在 Herdr 窗格内的进程。

在 Unix 上，每次创建窗格前都会校验 `SSH_AUTH_SOCK`：路径必须存在、是 socket 且属主为当前用户。当 server 继承的 agent socket 已失效（例如宿主终端重启后），Herdr 回退到最近一次前台 client attach 时上报的值；两者都不可用时则不注入该变量，让窗格内的 `ssh` 明确报「无 agent」，而不是连上一个死 socket 卡死。

shell 可以用 `OSC 7`（`file://<host>/<path>`）上报工作目录。`<host>` 为空、`localhost` 或本机 hostname（大小写无关）时 Herdr 接受该上报；指向其它主机的上报按远程处理并忽略。被接受的目录驱动 `new_cwd = "follow"`，并作为窗格 cwd 显示在侧边栏与 `herdr pane list` 中。

Herdr 也会把焦点窗格的工作目录以 `OSC 7`（`file://<host>/<percent 编码路径>`）回传给宿主终端。上送只在焦点或目录变化时发生，不随帧重复。理解 `OSC 7` 的宿主终端（WezTerm、kitty、foot 等）会在该目录下打开新标签页/分屏，因此用 `Ctrl+Shift+T` 新建的宿主标签页会落在焦点窗格所在目录。
...
Git 工作区行提供 worktree 操作。`New worktree` 会创建检出：如果输入的分支已存在，则检出该本地分支，否则创建该分支；随后将其作为新的 Herdr 工作区打开，并归组到来源工作区下。`Open worktree...` 会列出该仓库现有的 Git worktree 检出；选择已打开的检出会聚焦它，选择尚未打开的检出会在同一组中打开它。目录已删除但 Git 仍登记的检出会保留在列表中并标注「待清理」；选择它时会提示运行 `git worktree prune`，而不会尝试打开；若该检出仍作为 Herdr 工作区打开着，选择它仍会聚焦该工作区。

归组的 worktree 仍像普通 Herdr 工作区一样工作：可以聚焦、重命名和关闭，也可以拥有自己的标签页和窗格。父行是原始工作区。关闭父行会关闭整个 Herdr 组，但不会删除检出目录或分支。

删除 worktree 检出需要显式操作。在归组的子工作区上使用 `Delete worktree checkout...` 来运行 `git worktree remove`。Herdr 会先请求 Git 安全删除。如果 Git 因检出中存在已修改或未跟踪的文件而拒绝，Herdr 会在强制删除前再次确认。这一步的确认键故意不是回车：需要按 `y`（或 `ctrl+↵`），或点击「y 仍然删除」，这样上一步长按或连点的回车穿不到强制删除；该步的默认强调按钮是取消。分支不会被删除。
...
默认键位以前缀为主，因此 Herdr 不会抢占 shell、编辑器、tmux 或终端应用的输入。在[配置参考](/docs/config-reference/)中搜索 `keys.` 可查看每个动作及其默认绑定。应用内帮助面板可通过 `prefix+?` 打开，其中会显示当前生效的绑定。前缀模式激活时会弹出 which-key 面板列出可用的前缀绑定（可用 `ui.which_key = false` 关闭）。`prefix+u` 进入链接提示模式：窗格视口中可见的 URL 会显示两字母标记，输入标记即可打开链接。
...
`terminal` 主题把 Herdr 的结构色映射到 ANSI 调色板：输入框、键帽与未聚焦标签用的弱结构面是 `DarkGray`，更强的 hover / 活动面是 `Gray`，而正文与面板底色仍跟随终端自身的颜色。因此输入框、hover 高亮与禁用按钮在 16 色终端上仍有可见边界，不会塌缩成普通文本。
...
`sidebar_bg` 可单独设置桌面侧边栏的背景色；停靠布局下工作区列表与 Agents 面板都使用它。未设置时，默认的 `catppuccin` 主题用它的面板色铺满侧边栏，其它内置主题则继续使用宿主终端背景。`active_row_bg` 可更改当前 Space 和已聚焦 Agent 行的背景色，而不会影响分隔线或滚动条轨道。`selection_bg` 可更改侧边栏中 Navigate 模式光标行的背景色。
...
组件级 token 在已解析的调色板之上微调单个 UI 元素，未设置的字段会回落到对应的语义 token：
...
未设置的字段保留语义回落：窗格边框使用 `accent`/`overlay0`，滚动条滑块使用 `overlay1`（聚焦）或 `overlay0`，滚动条轨道使用 `overlay0`（聚焦）或 `surface_dim`，toast 边框使用 `green`/`blue`/`red`，模式栏使用 `accent`，`hover_bg`（菜单与浮层列表中指针悬停的那一行）取主题里第一个与面板背景明显可辨的表面色——多数主题是 `surface1`，`surface1` 与 `panel_bg` 几乎同色的浅色主题会自动退到更强的一档。`selection_mix_ratio`（0.0–1.0，默认 0.28）控制选区背景向白色或黑色混合宿主背景色的强度；被选文本的粗体、斜体和下划线样式会保留，只有单元格颜色改变。完整字段列表请在[配置参考](/docs/config-reference/)中搜索 `theme.components`。
...
`ui.border_style` 可在 `"single"`（默认）、`"rounded"`、`"double"`、`"thick"` 之间切换窗格边框字形。

`ui.color_depth` 控制 Herdr 自身 UI 颜色的输出色深：`"auto"`（默认）跟随宿主终端——当 TERM 为 `256color` 且没有 truecolor 证据时（SSH 或 tmux 中常见），主题 RGB 颜色会自动映射到最近的 xterm-256 调色板条目。使用 `"truecolor"` 强制 24 位输出，或使用 `"256"` 强制降级。

鼠标交互时序可调：`ui.double_click_ms`（默认 `350`）是双击选词、三击选整行的时间窗，也是片段列表里「同一个片段的第二次点击才启动运行」的时间窗；`ui.drag_throttle_ms`（默认 `33`）节流窗格滚动条与分割条的拖拽更新；`ui.selection_autoscroll_interval_ms` / `ui.selection_autoscroll_min_lines` / `ui.selection_autoscroll_max_lines`（默认 `30` / `3` / `15`）控制拖过窗格边缘时的选区自动滚动。

输入成帧窗口同样可调：`ui.input_sequence_timeout_ms`（默认 `150`）是客户端等待被读取边界切开的不完整控制序列剩余字节的时长——按键编码、鼠标报文或裸 `ESC [` 引导符都算——超时后丢弃该片段（因此真正的 Alt+[ 按键也要等这个窗口才送出）；`ui.escape_after_mouse_timeout_ms`（默认 `30`）是鼠标捕获开启（或最近半秒内收到过鼠标报文）时，孤立 Esc 等待可能的鼠标报文续接的时长，否则 Esc 在 10 ms 空闲窗口后即送出。SSH 高延迟链路可将两者调大；两者取值被钳制在 `10..=1000`，都在客户端启动时读取，且仅对 Unix 客户端生效（Windows 客户端固定按 10 ms 轮询）。
...
- `machine` — 客户端连接多台机器时显示机器名称；只有一台本地机器时省略。
- `workspace` — 工作区名称。
- `tab` — 标签页名称（如有）。
- `pane` — 窗格名称（如有）。
...
只有第一条匹配的规则生效。指定的样式字段覆盖当前位置的默认值，未指定的字段保持继承。没有规则匹配时保留默认样式。匹配使用显示截断前的完整值。在规则中设置 `hide = true` 可移除匹配的 token 及其分隔符；没有剩余 token 的行也会消失。例如，`{ token = "machine", fg = "#61afef", rules = [{ equals = "Local", hide = true }] }` 会在机器名称恰好为 `Local` 时隐藏机器 token。它比较的是名称，而不是连接类型。设置 `hide = false` 或省略时，token 保持可见。没有覆盖字段的规则一旦匹配，仍会停止后续匹配并保留默认值。
...
侧边栏行设置仅影响展开的桌面侧边栏。折叠视图和移动视图仍使用紧凑布局。在按分组显示的 agent 树里，上方分组头已经显示的机器、工作区或标签页，agent 行不再重复，即省略对应的 `machine`、`workspace`、`tab` token；因此只剩 `state_icon` 的那一行会并入下一行。所有行都不含 `state_text` 时，放得下的话树会在 agent 名称后补上状态文字。
...
选择 `herdr` 可使用应用内 toast，选择 `terminal` 可使用适合 SSH 场景的外层终端通知，选择 `system` 可使用本地操作系统通知服务，选择 `off` 可禁用弹出通知。Herdr 会抑制活动标签页的弹出通知。在[配置参考](/docs/config-reference/)中搜索 `ui.toast`，可查看位置、延迟行为和剪贴板反馈设置。 通知横幅与 toast 会带上来源窗格的显示名（没有显示名时用 pane id），移动端窄屏下也能认出是谁在通知。
...
按智能体覆盖声音设置时，可使用 `default`、`on` 或 `off`。每家官方智能体一个键:`pi`、`claude`、`codex`、`open_code`(OpenCode,注意带下划线)和 `kimi`。指向[已退役智能体](/zh-cn/docs/integrations/#已退役的集成)的键会被忽略并给出 `unknown config key` 诊断,同段其余设置照常生效。
...
隐藏硬件光标的 AI 智能体 TUI 会让宿主终端失去光标锚点：输入法候选窗口不再跟随聚焦窗格，打字插入符也会中途消失。因此 Herdr 默认显示一个闪烁竖条锚点；可按需关闭或收窄：

```toml
[experimental]
reveal_hidden_cursor_for_cjk_ime = true # 默认开启
cjk_ime_cursor_shape = "bar"            # 默认闪烁竖条
cjk_ime_agents = []                     # 留空 = 任意聚焦窗格
```

限制 `cjk_ime_agents` 可以避免在无关应用中额外显示硬件光标。在[配置参考](/docs/config-reference/)中搜索这些键，可查看接受的智能体名称和光标形状。

应用用同步输出（`DECSET 2026`）包裹重绘时，批次未结束期间保持上一帧的光标状态而不是隐藏光标，宿主不会每帧收到一对隐藏/显示序列。批次超过 1 秒仍未复位视为失效：Herdr 恢复渲染，光标重新跟随窗格，因此从不复位该模式的 TUI 也不会让宿主终端失去光标。

Herdr 自己的每一帧都包在同步输出批次里，并默认在批次结束后再补发一次最终光标锚点，让忽略 `DECSET 2026` 块内光标移动的原生输入法仍能跟随插入符。在会把这次多出来的裸光标移动显示出来的终端上，它只会表现为光标或切换标签时的闪烁，因此 `ui.repeat_ime_cursor_anchor` 在客户端启动时判定一次：
...
`auto` 在 `TERM_PROGRAM`、`TERM` 或 `KITTY_WINDOW_ID` 标识出已知会把补发显示成闪烁的终端（WezTerm、kitty、ghostty、foot、contour、iTerm2、Alacritty、rio）时跳过补发；未知宿主以及 tmux、screen 内保留补发——多路复用器会继承外层终端的身份，但其同步输出透传行为随版本变化。判定只看终端名不看版本：若在上述终端的旧版本里输入法丢失插入符，设为 `always`；若未知宿主本身已能原子呈现同步输出，设为 `never` 去掉补发。Windows 无论如何都不补发。该设置只影响客户端自渲染 shell 的帧；`herdr terminal attach` 会话由服务端渲染，保持默认补发。
...
| `GIT_CEILING_DIRECTORIES` | 与 Git 语义一致：工作区标签、分支标记和 worktree 操作所用的仓库发现，在列出的目录之下停止向上查找。 |
...
日志会自动轮转：单个文件上限 5 MB，并在同级保留最近两份轮转副本（`herdr-server.log.1`、`herdr-server.log.2` 等）。报告问题时，请附上当前日志和轮转后的同级日志文件。

...
鼠标交互时序可调：`ui.double_click_ms`（默认 `350`）是双击选词、三击选整行的时间窗；`ui.drag_throttle_ms`（默认 `33`）节流窗格滚动条与分割条的拖拽更新；`ui.selection_autoscroll_interval_ms` / `ui.selection_autoscroll_min_lines` / `ui.selection_autoscroll_max_lines`（默认 `30` / `3` / `15`）控制拖过窗格边缘时的选区自动滚动。

...
`pane.process_info` 返回窗格的 shell pid、可用时的前台进程组 id,以及平台暴露时带有 pid、名称、argv/cmdline 和 cwd 的前台进程。argv/cmdline 里疑似凭据的值会替换成 `[REDACTED]`:`--token`、`--api-key` 等选项的值,`Authorization:` 等敏感请求头(保留 `Bearer`/`Basic` 等认证方案名),`Bearer` 令牌,`*_KEY=`、`*_TOKEN=` 这类赋值,以及 URL 里的口令与凭据类查询参数。程序名与参数结构保持原样,调用方仍能看出在运行什么。

...
普通的 `herdr api schema` 打印简短摘要。`--json` 打印供工具使用的完整 JSON Schema,`--output PATH` 将该文档写入文件。schema 覆盖原始请求、成功响应、错误响应、发出的事件和订阅事件。`events.subscribe` 流上推送的行由 `stream_event` 条目(事件帧加可选 `sequence`)与 `event_stream_notice` 条目(流的元信息,例如 `events.lost`)描述;`event` 条目描述的是事件载荷本身。
...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear`、`agent.activity.read`、`agent.external.list` |
...
| 观测 | `system.metrics.get`、`system.metrics.subscribe`、`system.metrics.unsubscribe`、`system.process.list`、`system.process.get`、`system.process.terminate`、`account.usage.providers`、`account.usage.get`、`account.usage.refresh`、`account.usage.subscribe`、`account.usage.unsubscribe`、`account.usage.report`、`account.usage.integration`、`account.binding.set` |
...
`pane.close` 在窗格被摘除后立即返回。Herdr 在后台按 SIGHUP、SIGTERM、SIGKILL 的顺序终止窗格内的进程,赖着不退的进程不再拖住同一连接或其它连接上的请求。需要确认这些进程已退出的调用方应轮询检查(例如对仍存活的窗格调用 `pane.process_info`,或自行检查进程),而不是假定响应返回时它们已经结束。
...
`pane.process_info` 返回窗格的 shell pid、可用时的前台进程组 id,以及平台暴露时带有 pid、名称、argv/cmdline 和 cwd 的前台进程。argv/cmdline 里疑似凭据的值会替换成 `[REDACTED]`:`--token`、`--api-key` 等选项的值,`Authorization:` 等敏感请求头(保留 `Bearer`/`Basic` 等认证方案名),`Bearer` 令牌,`*_KEY=`、`*_TOKEN=` 这类赋值,以及 URL 里的口令与凭据类查询参数。http(s) URL 里不带口令的用户信息通常就是令牌,会整段打码(`https://user@bitbucket.org/…` 显示为 `https://[REDACTED]@bitbucket.org/…`);`ssh://git@…` 等其它协议保留用户名。curl 的 `-u 名:口令` 这类写法只打口令部分。程序名与参数结构保持原样,调用方仍能看出在运行什么。

同一过滤还覆盖 `--creds`、shell 请求头中的转义空格、PowerShell 请求头带空格的赋值，以及 Git `http.extraHeader` 中的认证值。进程把程序名改写成整条命令行时，`argv0`、`argv` 与 `cmdline` 中符合凭据形态的参数都会打码；普通可执行文件路径（包括带空格的路径）保持不变。

对于 curl 与 curlie，明确的数据和 JSON 选项还会过滤表单及嵌套 JSON 中的凭据字段，保留非敏感字段。正文超过 64 KiB、JSON 深度超过 16 层或格式不完整时整体遮蔽；绝不读取 `@file` 引用。`--data-raw` 与 `--form-string` 中字面的 `@` 值仍会过滤；multipart 表单和 URL 编码选项按单个 `name=value` 处理，不把值内的 `&` 拆成其它字段。绝对与相对 URL 的查询及 fragment 参数都会处理，包括 OAuth 的 `code`；普通 `--code` 命令选项保持不变。这是诊断输出过滤，不保证任意终端原文都不含秘密；窗格原文读取、布局导出与会话恢复数据保持已有行为。
...
Worktree 方法把 Git 检出作为 Herdr 工作区管理。`worktree.create` 创建检出,并返回新的 `workspace`、`tab`、`root_pane` 和 `worktree` 记录。请求的分支已在本地存在时检出它;否则从请求的 base 或 `HEAD` 创建分支。`worktree.open` 打开已有检出,或返回已打开的工作区。`worktree.remove` 对关联的子工作区运行 `git worktree remove`,从不删除分支。`worktree.list` 结果中的 `is_prunable` 标记目录已缺失但 Git 仍登记的关联检出;Herdr 在任何 Git 版本上都根据检出状态推导它,而不只依赖 Git 2.31+ 才输出的 `prunable` 行。
...
智能体的活动树(子智能体、任务、待办、后台进程)是服务器拥有的事实。服务器在
后台线程里通过每种智能体各自的来源适配器读取它;钩子只负责发出「可能变了」的
信号。

读取智能体自身会话文件的适配器(Claude Code、Codex、Kimi Code)起初按 Herdr
服务器环境中的 `CLAUDE_CONFIG_DIR`、`CODEX_HOME` 或 `KIMI_CODE_HOME` 定位配置目录,
没有覆盖时使用 `~/.claude`、`~/.codex` 或 `~/.kimi-code`。Claude Code 和 Codex
会话钩子还会随会话 ID 上报绝对转录路径。只要窗格当前的智能体和会话 ID 仍匹配,
适配器就优先使用该路径,因此仅在窗格里设置 `CLAUDE_CONFIG_DIR` 或 `CODEX_HOME`
也能生效。resume、fork 上报会更新此关联;新 ID 未带有效路径时回退到服务器配置目录。
恢复命令仍使用会话 ID。Kimi Code 仍需在服务器环境中设置目录覆盖。

Codex 路径的文件名必须为 `rollout-<时间戳>-<会话 ID>.jsonl` 或 `.jsonl.zst`,
位于 `sessions/YYYY/MM/DD/` 或平铺的 `archived_sessions/`。未知布局不受支持,
Herdr 不会从任意父目录猜测会话库。活动目录优先于归档副本,同一 rollout 的普通
文件优先于压缩副本。发现与内容读取均支持只读压缩文件,保留原始 JSONL 字节游标,
不会解压落盘或改写源文件。

每个压缩文件最多读取 16 MiB 输入、64 MiB 解压内容、8 MiB 解码窗口及 128 帧。
每次发现/读取有 64 MiB 压缩输入与 72 MiB 解码工作预算（含未完成帧的窗口预留）。
两个 worker 共用一个冷解码器和最多 2,048 项的 128 MiB 缓存；正文淘汰后仍缓存
未变化的元数据、终态和失败结果,文件变化会使缓存失效。文件超限记为不支持；
共享或单次工作预算不足时稍后重试,不可读的生命周期保持未知。发现最多检查
8,192 个日期目录项、8,192 个文件候选,再读取其中 2,048 个选定 rollout。

Codex 0.157 回退后的线程可在稳定会话 ID 后追加 `_<rollout-id>`。上报仍匹配稳定 ID;发现和内容读取先按文件名时间、再按 rollout ID 选最新版本,每个线程仅显示一次。活动读取展示所选 rollout,暂不展开指向早期 rollout 的 `history_base` 历史前缀。

Codex 游标还携带逻辑 rollout 身份。回退切换到另一份 rollout 时,`activity_cursor_stale` 表示调用方应清除已累计内容并不带游标重读；TUI 在下一次 tick 执行该操作。压缩或移入归档保持同一身份。为兼容仍接受旧数字游标,但它无法检测文件替换；请始终原样回传服务器给出的不透明 `next_cursor`。临时文件访问失败会重试,不会作为格式损坏缓存。


钩子用 `pane.report_agent_activity` 发信号:
...
`hint` 可选。它可以是钩子事件名之类的自由文本,也可以是该智能体的来源适配器能
理解的结构化快照:Herdr 的 Pi 扩展就是这样把整棵树作为 `herdr.activity.snapshot`
JSON 文档上报的,因为 Pi 不在磁盘上保留任何活动记录。服务器只保留每个窗格最近
一份 hint,并在下一次刷新时交给来源适配器。整条请求(含 hint)必须在 API 的
1 MiB 请求上限之内;超出的请求会被直接丢弃,没有响应,也不会安排刷新。`node_id` 与 `seq` 仅用于诊断。响应是
`ok`,从不携带树;除 Pi 这类快照 hint 外,请求里也不带树。

一个 hint 会立即安排一次刷新,但同一窗格每秒最多刷新一次;约 2 秒后还会再补刷
一次(期间若已有别的刷新则不再补刷),因为钩子往往在智能体把变化写入磁盘之前就已
触发。与 hint 无关地,服务器在
首次检测到智能体时刷新一次,在其状态为 `working` 期间每 5 秒刷新一次,并在工作
结束时再补刷一次(约 2 秒后同样再补一次)。只要已存储的树里还有 `kind` 不是
`todo`、状态为 `pending`、`running` 或 `blocked` 的节点,智能体空闲后服务器仍每
5 秒刷新一次,晚于父回合结束的子智能体因此也能到达最终状态;这些节点都结束、或
这棵树 10 分钟没有变化后停止。每次刷新每个智能体最多保留 32 个节点(运行中的
节点及其祖先优先),并记下截断前这棵树有多大。

`agent.get`、`agent.list` 和 `session.snapshot` 以 `AgentInfo.activity` 暴露已存储的
树,为空时省略。每个节点带 `id`、`kind`(`subagent`、`task`、`todo`、`background`、
`unknown`)、`label`、`status`(`pending`、`running`、`blocked`、`done`、`failed`、
`unknown`),以及可选的 `parent_id`、`agent_type`、`started_at_ms`、`ended_at_ms`、
`content_ref` 和 `summary`。未知枚举值解码为 `unknown`。
`AgentInfo.activity_running`、`activity_total` 和 `activity_truncated` 描述的是
来源报出的、截断前的那棵树;三者各自在为 0 或 false 时省略,所以智能体没有活动时
三者都不出现。`activity_truncated`
置位时 `activity` 里最多只有 32 个节点,按来源顺序排列:先保留运行中的节点及其祖先,
再补父节点已被保留的其它节点;整棵树用不带 `node_id` 的 `agent.activity.read` 拉取。`AgentInfo.launch_seq` 是
服务器分配的启动顺序:窗格首次获得智能体身份时得到一个新的单调序号,状态变化
不改号,智能体被释放或重新识别后取新号。`0` 表示未知(旧服务器不下发该字段);
序号只在同一个服务器进程内可比。

用 `agent.activity.read` 读取树或某个节点的内容:
...
`pane_id` 与 `external_id` 恰好给一个,否则调用以 `invalid_params` 失败。不带
`node_id` 时响应在 `nodes` 里带整棵树(目标是 `pane_id` 时这次读取同时刷新已存储的树,
下一份快照与之一致,除非已存有开始得更晚的刷新结果。已存储的树与列表按读取开始的先后排序,而不是按到达的先后:比这次读取
开始得早的该窗格后台刷新晚到时作废,比它开始得晚的照常覆盖它。
目标是 `external_id` 时刷新的是该来源的列表,列表里这个条目可能仍只有这棵树的一部分);带 `node_id` 时 `nodes` 为空,响应带一个 `content` 片段:`node_id`、`format`
(`text`、`markdown`、`jsonl`、`unknown`)、`text`、`eof`、`truncated`,以及下一页
要作为 `cursor` 回传的 `next_cursor`。`cursor` 对调用方不透明。`max_bytes` 默认 65536,上限 524288。
`follow: true` 让服务器在接下来的 10 秒内每秒刷新该窗格的树,轮询的查看器可以借此
跟踪运行中的节点;它不改变内容读取的起始位置,续读仍靠 `cursor`,`external_id`
目标忽略它。响应形如
`{"type":"agent_activity","nodes":[...],"content":{...}}`,由后台读取线程产生,
因此可能晚于后续请求的响应到达。

错误:`invalid_params`(除上面的目标规则外,还有 `node_id` 为空、`max_bytes: 0` 或
`external_id` 格式不对);`pane_not_found`;`agent_not_found`(窗格没有智能体、外部来源
未知,或读整棵树时该来源已不再列出这个外部会话);
`not_implemented`(本服务器没有该智能体的来源适配器,或适配器无法应答这次请求,
例如 Herdr 还没有收到该智能体的原生会话引用);`activity_unavailable`
(来源此刻不可读,例如 Pi 还没有上报过快照时读取某个节点);`activity_malformed`(来源内容或
游标不是预期格式);`activity_io_error`;读取队列已满时 `server_busy`;后台读取线程没有运行时
`server_unavailable`。

Herdr 为 Claude Code、Codex、Kimi Code 和 OpenCode 提供来源适配器,读取各智能体
自己的本地会话数据;Pi 的适配器读取 Herdr 的 Pi 扩展上报的快照。没有适配器的
智能体(例如以自定义名字上报的)回 `not_implemented`。Claude Code、Codex 和
OpenCode 靠 Herdr 为该窗格存下的原生会话引用定位会话(通常由 Herdr 集成在会话
开始时上报),没有引用时树保持为空;Kimi Code 则退而取窗格工作目录下最近活动的
会话。OpenCode 适配器运行服务器 `PATH` 上的 `opencode` 命令,并跟随服务器环境的
`XDG_DATA_HOME`;找不到该命令时读取以 `activity_io_error` 失败。

`agent.external.list`(无参数)列出不属于任何窗格的智能体会话,响应为
`{"type":"external_agent_list","agents":[...]}`,条目是 `ExternalAgentInfo`:
`external_id`(`<source>:<会话 id>`,可作为 `agent.activity.read` 的
`external_id`)、`source`、`agent_status`、`label`、`readable`(`false` 表示该条目的
来源已有 60 秒没能成功读取),以及可选的 `agent`、
`cwd`、`updated_at_ms` 和 `activity`。目前唯一的外部来源是 `zcode`,即 ZCode
桌面应用:它通过系统的 `sqlite3` 命令只读 ZCode 的本地数据库,列出最近 72 小时内
更新过、未归档的根会话,最多 10 个,每个带其子智能体树与待办。这 10 个根共用
同一份 300 行上限,排在前面的根子树很大时,后面的根在列表里可能只剩根自身;
之后按某个根的 `external_id` 用 `agent.activity.read` 读整棵树时,改成只针对
该根的独立查询,独享一份 300 行上限,不与列表共享。没有 ZCode 的
数据库(未安装或从未启动过 ZCode)时调用成功返回空列表;客户端拿到空列表就隐藏
外部条目区。没有 `sqlite3` 或数据库读不了时,调用以 `activity_unavailable` 失败。
只有当所有已注册来源都报告无法枚举外部会话时才回 `not_implemented`。

刷新改变了已存储的树时,服务器发出 `pane.agent_activity_changed`,带 `pane_id`、
`running` 和 `total`。该事件只带计数;树要用 `agent.get` 或 `agent.activity.read`
读取。订阅该事件不接受 `pane_id` 过滤,请按事件里的 `pane_id` 自行筛选。
...
第一个响应确认订阅。之后的行是推送的事件。生命周期事件订阅从请求被接受时开始,不会重放在此之前保留的事件。一批生命周期事件会在同一次投递中作为连续多行到达,而不是每个轮询间隔一行。投递仍由轮询循环驱动,因此一批事件中第一行的延迟上限仍是一个轮询间隔。基于快照的订阅(`pane.output_matched`、`pane.agent_status_changed`、`pane.scroll_changed`)是边沿触发,每个轮询间隔至多产出一条事件。在订阅连接上写入内容(例如换行保活)不会结束该流;这些字节会留在缓冲里不被消费,直到连接关闭。

每一行生命周期事件都带有可选的 `sequence`,即该事件在本 server 事件序列中的位置。可以用它对账事件顺序、判断流推进到哪里。基于快照的帧没有序号,会省略该字段;旧版 server 在所有位置都省略它。

server 只保留有界的近期事件环形缓冲。如果订阅方落后到**它订阅的**事件在投递前就被挤掉,流可以在幸存事件之前先发一行通知:

```json
{"event":"events.lost","data":{"from":601,"to":688}}
```

通知行需要显式开启:在 `events.subscribe` 的参数里设 `notices: true` 才会收到;该字段默认 `false`,省略它的订阅看到的行与以前完全一致。只有本订阅关心的事件种别真被挤掉才会报断层——无关流量滚过缓冲不会触发;同一连接即使多个订阅同时受影响,每个轮询轮次也至多收到一条通知。

`from` 与 `to` 是你订阅的事件被丢弃且不可恢复的序号闭区间。把它当作重新同步信号:重新读取 `session.snapshot`(或你自己缓存的状态),然后继续应用后续事件流。

这条通知是新增的行类型,不改变既有事件帧的形状。由此把 `events.subscribe` 流的契约写明:每一行要么是 `stream_event`(带可选 `sequence` 的事件帧),要么是 `event_stream_notice`,客户端**必须**忽略无法识别的 `event` 名,而不是当成错误。`events.lost` 不是事件种别,所以严格按 `event` 条目解码每一行的客户端要么不开启通知(默认),要么按 `stream_event` 与 `event_stream_notice` 一起解码。
...
观测数据走专用方法而不是 `events.subscribe`：`account.usage.subscribe` 的参数与 `account.usage.get` 相同，响应返回 `subscription_id`，之后向该连接推送 `account.usage.updated`（变化的账号快照及其 `refresh` 状态）和 `account.usage.refreshing`（探测刚派发时只推刷新状态），直到 `account.usage.unsubscribe`；`system.metrics.subscribe` 同样推送 `system.metrics.updated`。订阅者会驱动服务端在后台持续探测，数据不再展示时请退订。订阅随连接消亡，重连后需要重新订阅。generation-1 客户端端点以 `endpoint.observation.v1` 控制帧接收这些推送，帧内带 server 的 `boot_id` 与事件；未知事件种类会被忽略而不是拒绝。

观测推送使用与 `events.subscribe` 相同的 `{"event": "<名称>", "data": {...}}` 信封。`account.usage.updated` 携带 `accounts`（变化的 `AccountUsageSnapshot` 条目）和可选的 `refresh` 列表；`account.usage.refreshing` 只携带 `refresh`；`system.metrics.updated` 携带 `snapshot`。`refresh` 条目（`UsageRefreshState`）同样出现在 `account.usage.get` 与 `account.usage.refresh` 的响应里，按 `account_id` 与 `accounts` 对齐；旧版 server 省略该列表。字段含义：`in_flight` 与 `queued`（探测已派发 / 仍在等待查询线程）；`requested_at_ms` 与 `attempted_at_ms`（最近一次显式刷新——含被防抖的——与最近一次真正启动的探测）；`next_allowed_at_ms`（下一次显式刷新会被接受的最早时间，缺省表示现在即可）；`retry_after_ms`（HTTP 429 等厂商退避，显式刷新也不豁免）；`binding_inferred`（请求带了未绑定窗格，数据按该厂商唯一配置的账号返回，绑定并未写入）；`trust_required`（交互探测停在了 CLI 的目录信任对话上，快照 `message` 给出目录）；`callback_only`（样本只来自推送：官方回调，或 Pi 的 Herdr 集成扩展）；`pending_binding`（未绑定窗格的回调被拒后留下的 `pane_id`、`agent`、`candidates`、`rejected_at_ms`）；`callback_enabled`（官方回调是否已接入 herdr，未知或厂商没有回调时——例如 Pi——缺省）。`binding_inferred` 与 `pending_binding` 按请求填写，事件里保持缺省，以 `account.usage.get` 的响应为准。
...
带编号的二进制协议仍用于同一安装内的内部操作,包括直接终端连接和实时交接。跨不同构建使用这些操作前,请检查 `ping` 或 `herdr status`。JSON API 客户端应忽略未知字段,并把不支持的方法当作普通错误处理。在推送流上还必须忽略无法识别的 `event` 名:新的行类型(例如 `events.lost`)会从那里加入,而是否接收它们由 `events.subscribe` 的 `notices` 开关控制。

generation-1 端点内的追加同样保持可选:客户端可在握手中上报宿主 `ssh_auth_sock`,供 detached server 在宿主终端重启后兜底;焦点 pane 的目录变化时,server 推送 `endpoint.terminal-cwd.v1` 控制帧,由客户端写宿主终端的 OSC 7。未知的控制帧种类、未知的握手字段与未知的事件名一律忽略,而不是拒绝。

## 不可变文字快照

server 分别声明 `pane.text_snapshot.capture`、`pane.text_snapshot.read`、`pane.text_snapshot.selection`、`pane.text_snapshot.retain` 和 `pane.text_snapshot.release`，不改变 generation 1 二进制 codec。capture 接收 `pane_id`，返回 server boot、快照 ID、带样式的初始窗口和绝对行坐标。保留范围与截断标记说明历史边界；read 接收 `snapshot_id`、`start_row` 和 `rows`。selection 接收 ID 以及包含端点的 `anchor`、`cursor` 坐标，只读取固定快照。

TUI 快照属于创建它的连接。retain 确认客户端已安装快照；未确认的捕获 60 秒后过期，已确认的快照随连接或表面释放。原始 JSON API 快照闲置五分钟后过期。release 可重复调用。调用方需处理单快照/全局容量以及 `snapshot_capacity`、`snapshot_expired`、`snapshot_range`、`snapshot_unavailable`，不能静默改为复制实时内容。

...
一份 hint,并在下一次刷新时交给来源适配器。`node_id` 与 `seq` 仅用于诊断。响应是
...
置位时 `activity` 里只剩运行中的节点及其祖先,其余用 `agent.activity.read`
(不带 `node_id`)拉取。`AgentInfo.launch_seq` 是
...
`not_implemented`(本服务器没有该智能体的来源适配器);`activity_unavailable`
(来源此刻不可读,例如 Pi 还没有上报过快照);`activity_malformed`(来源内容或
游标不是预期格式);`activity_io_error`;读取队列已满时 `server_busy`。目前只有 Pi
(通过 Herdr 的 Pi 扩展)提供树;其他智能体在各自的来源适配器落地前回
`not_implemented`。
...
`cwd`、`updated_at_ms` 和 `activity`。目前还没有任何外部来源适配器,该调用成功
返回空列表;客户端拿到空列表就隐藏外部条目区。只有当所有已注册来源都报告无法
枚举外部会话时才回 `not_implemented`。

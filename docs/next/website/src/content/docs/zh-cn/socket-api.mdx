...
观测推送使用与 `events.subscribe` 相同的 `{"event": "<名称>", "data": {...}}` 信封。`account.usage.updated` 携带 `accounts`（变化的 `AccountUsageSnapshot` 条目）和可选的 `refresh` 列表；`account.usage.refreshing` 只携带 `refresh`；`system.metrics.updated` 携带 `snapshot`。`refresh` 条目（`UsageRefreshState`）同样出现在 `account.usage.get` 与 `account.usage.refresh` 的响应里，按 `account_id` 与 `accounts` 对齐；旧版 server 省略该列表。字段含义：`in_flight` 与 `queued`（探测已派发 / 仍在等待查询线程）；`requested_at_ms` 与 `attempted_at_ms`（最近一次显式刷新——含被防抖的——与最近一次真正启动的探测）；`next_allowed_at_ms`（下一次显式刷新会被接受的最早时间，缺省表示现在即可）；`retry_after_ms`（HTTP 429 等厂商退避，显式刷新也不豁免）；`binding_inferred`（请求带了未绑定窗格，数据按该厂商唯一配置的账号返回，绑定并未写入）；`trust_required`（交互探测停在了 CLI 的目录信任对话上，快照 `message` 给出目录）；`callback_only`（额度样本只来自官方回调）；`pending_binding`（未绑定窗格的回调被拒后留下的 `pane_id`、`agent`、`candidates`、`rejected_at_ms`）；`callback_enabled`（官方回调是否已接入 herdr，未知时缺省）。`binding_inferred` 与 `pending_binding` 按请求填写，事件里保持缺省，以 `account.usage.get` 的响应为准。

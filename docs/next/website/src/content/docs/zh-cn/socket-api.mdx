...
一份不超过 1 MiB 的 hint,并在下一次刷新时交给来源适配器;更大的 hint 仍会安排
一次刷新,但保留的仍是上一份。`node_id` 与 `seq` 仅用于诊断。响应是
`ok`,从不携带树;除 Pi 这类快照 hint 外,请求里也不带树。

一个 hint 会立即安排一次刷新,但同一窗格每秒最多刷新一次;约 2 秒后还会再补刷
一次,因为钩子往往在智能体把变化写入磁盘之前就已触发。与 hint 无关地,服务器在
...
来源报出的、截断前的那棵树;智能体没有活动时三者都省略。`activity_truncated`
置位时 `activity` 里只有保留下来的 32 个节点(运行中的节点及其祖先优先,其余按
来源顺序补足),整棵树用不带 `node_id` 的 `agent.activity.read` 拉取。`AgentInfo.launch_seq` 是
...
一致);带 `node_id` 时响应带一个 `content` 片段:`format`(`text`、`markdown`、
`jsonl`、`unknown`)、`text`、`eof`、`truncated`,以及下一页要作为 `cursor` 回传的
`next_cursor`。`cursor` 对调用方不透明。`max_bytes` 默认 65536,上限 524288。
...
错误:`pane_not_found`;`agent_not_found`(窗格没有智能体、外部来源未知,或该来源已不再列出这个外部会话);
`not_implemented`(本服务器没有该智能体的来源适配器,或适配器无法应答这次请求,
例如 Herdr 还没有收到该智能体的原生会话引用);`activity_unavailable`
(来源此刻不可读,例如 Pi 还没有上报过快照);`activity_malformed`(来源内容或
游标不是预期格式);`activity_io_error`;读取队列已满时 `server_busy`。
...
`external_id`)、`source`、`agent_status`、`label`、`readable`,以及可选的 `agent`、

...
只有你显式关闭最后一个工作区时,Herdr 才会清空 `session.json`。如果工作区集合因为别的原因归零,比如主机重启挂断了所有 shell,或者最后一个窗格进程自己退出,Herdr 会保留已保存的快照而不是删掉它,让下次启动仍然能恢复会话。恢复回来的是磁盘上最近的那份快照,它可能比窗格死掉的时刻早几秒。

另外,Herdr 会在窗格级联退出期间保护这份快照。只要有多个窗格在几秒内先后退出,或者某个 shell 报出主机关机时常见的退出码,Herdr 就会在移除该窗格之前先写一次检查点,并且不再用更小的工作区集合覆盖已保存的快照。这道护栏在最后一次这类退出之后约 90 秒解除,所以正常关掉若干 shell 仍然会被保存,只是稍晚一点。

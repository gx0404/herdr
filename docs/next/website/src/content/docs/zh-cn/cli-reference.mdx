...
`machine status` 在一份报告中展示单台机器：profile 字段、经机器 SSH API 桥的实时连接探测（`online`、`disabled`，或 `unreachable` 并附结构化失败类型与信息）、探测成功时的远程 server 版本与协议兼容性、已保存的端口转发规则，以及会话日志偏好。选择器接受标签或 profile id，包括已禁用机器。
...
`machine import` 一次性登记 SSH 配置中的全部 `Host`。不带 `--yes` 时会列出检测到的主机并询问导入哪些，因此需要交互式终端；脚本请传 `--yes` 全量导入。`--host` 将批次限制为匹配的别名或 `*`/`?` 模式，`--group` 为所有导入的机器指定同一分组，`*.internal` 这类通配主机除非传 `--include-wildcards` 否则跳过。引用同批导入主机的 `ProxyJump` 跳点会成为 `profile:<id>` 引用。与已保存机器标签或目标重复的主机会被跳过并逐台报告，因此重复执行是安全的。导入按 OpenSSH 首个值生效的语义读取 SSH 配置（包括 `Include` 引入的文件），绝不修改它；与 `machine add` 不同，导入不安装或更新远程服务器，仍需设置的机器会显示 Attention 并给出独立修复命令。完整语义见[连接机器](/zh-cn/docs/connecting-machines/#从-ssh-配置导入机器)。
...
`<profile>` 是保存机器的标签或 profile id，解析规则与 `--machine` 相同。远程路径由 sftp 服务器相对登录目录解析（不展开 `~`），不得以 `-` 开头，且不得包含 `\` 或 `"`。单文件传输上限 1 MiB；`get` 以 `0600` 权限原子写入，`put` 先上传再原子 rename 覆盖目标并恢复原权限位。`rm` 删除目录需 `--recursive`，不跟随符号链接。详见 [连接机器](/docs/connecting-machines/#remote-files)。

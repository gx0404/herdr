...
同一个片段库也驱动界面：命令面板的「命令片段」与「运行片段…」打开片段面板，可管理片段库（新建/编辑/删除），并引导一次运行：先选目标（当前窗格、任一在线机器的窗格、或每台选中机器各自聚焦的窗格），再填已声明的变量，最后确认渲染后的命令。运行通过已连接的端点会话发出，不经过额外 SSH 桥接；逐目标结果汇成一条提示（全部目标都接受时显示 ✓ 成功样式），并写入同一份历史（面板的「历史」页可查看）。

...
Windows integration installation supports all five official integrations: Pi, Claude Code, Codex, OpenCode, and Kimi Code CLI. Sub-agent activity signals are Unix-only: on Windows the Claude Code and Codex integrations install only their session hook and the Kimi Code CLI hook ignores activity events, so those agents' activity trees update only on Herdr's periodic refresh.

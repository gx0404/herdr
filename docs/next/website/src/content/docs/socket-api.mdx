...
next snapshot matches it; a background refresh of that pane still in progress
when the read returns is discarded instead of overwriting it, and another
refresh follows. For an `external_id` target it refreshes that source's list
instead, where the entry can still hold only part of this tree); with `node_id` it carries an empty `nodes` list and a `content` chunk:

...
hint per pane, up to 1 MiB, and hands it to the source adapter on the next
refresh; a larger hint still schedules a refresh, but the previous hint is kept.
...
second, plus one follow-up refresh about 2 seconds later, because hooks often
fire before the agent has written the change to disk. Independently of hints,
...
reported it, before truncation; all three are omitted when the agent has no
activity. When `activity_truncated` is set, `activity` holds only the 32 kept
nodes — running nodes and their ancestors first, then the rest in source order —
so fetch the whole tree with `agent.activity.read` without `node_id`. `AgentInfo.launch_seq` is
...
it); with `node_id` it carries a `content` chunk: `format` (`text`, `markdown`,
`jsonl`, `unknown`), `text`, `eof`, `truncated`, and `next_cursor` to pass back
as `cursor` for the next page. `cursor` is opaque to the caller. `max_bytes`
...
Errors: `pane_not_found`, `agent_not_found` (the pane hosts no agent, the
external source is unknown, or that source no longer lists the external
session), `not_implemented` (this server has no source adapter for that agent,
...
source cannot be read right now, for example Pi has not reported a snapshot
yet), `activity_malformed` (the source or the cursor is not in the expected
format), `activity_io_error`, and `server_busy` when the reader queue is full.
...
recently active session in the pane's working directory.
...
`agent.activity.read`), `source`, `agent_status`, `label`, `readable`, and

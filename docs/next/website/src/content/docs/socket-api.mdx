...
`nodes` (this read also refreshes the stored tree, so the next snapshot matches
it; a background refresh of that pane still in progress when the read returns
is discarded instead of overwriting it, and another refresh follows); with `node_id` it carries an empty `nodes` list and a `content` chunk:

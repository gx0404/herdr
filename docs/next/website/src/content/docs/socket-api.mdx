...
Codex paths must name `rollout-<timestamp>-<session-id>.jsonl` or `.jsonl.zst` and
use the `sessions/YYYY/MM/DD/` layout. An unknown layout is unsupported; Herdr
does not guess a library from arbitrary parent directories. A compressed root
path can locate the library and discover children with readable metadata, but
compressed child rollouts are not decompressed or included in discovery and
reading their content remains unsupported.

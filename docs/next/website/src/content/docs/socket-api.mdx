...
a pane, such as `CLAUDE_CONFIG_DIR=/path claude`, is not visible to the server,
so that agent's activity is not found; set the variable in the environment the
server starts from as well.

...
their ancestors first) and sets `truncated` when the source reported more.
...
`summary`. Unknown enum values decode as `unknown`. `AgentInfo.launch_seq` is
...
that polls can track a running node. The response is
...
optional `agent`, `cwd`, `updated_at_ms`, and `activity`. No external source is
implemented yet, so the method currently answers `not_implemented`.

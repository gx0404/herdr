...
events, and subscription events. Lines pushed on an `events.subscribe` stream
are described by the `stream_event` entry (an event frame plus an optional
`sequence`) and the `event_stream_notice` entry (stream metadata such as
`events.lost`); the `event` entry describes the event payload itself.
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_agent_activity`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear`, `agent.activity.read`, `agent.external.list` |
...
| Observation | `system.metrics.get`, `system.metrics.subscribe`, `system.metrics.unsubscribe`, `system.process.list`, `system.process.get`, `system.process.terminate`, `account.usage.providers`, `account.usage.get`, `account.usage.refresh`, `account.usage.subscribe`, `account.usage.unsubscribe`, `account.usage.report`, `account.usage.integration`, `account.binding.set` |
...
a process that refuses to exit no longer stalls other requests on the same
connection or on other connections. Callers that need those processes gone
should poll (for example with `pane.process_info` on a surviving pane, or their
own process check) instead of assuming they exited when the response arrived.
...
when the platform exposes them. Values in argv/cmdline that look like
credentials are replaced with `[REDACTED]`: values of options such as
`--token` and `--api-key`, sensitive headers such as `Authorization:` (the
`Bearer`/`Basic` scheme is kept), `Bearer` tokens, `*_KEY=`/`*_TOKEN=`
assignments, and passwords or credential query parameters in URLs. For http(s)
URLs, user info without a password is redacted as a whole because it is usually
a token (`https://user@bitbucket.org/…` becomes
`https://[REDACTED]@bitbucket.org/…`); other schemes such as `ssh://git@…`
keep the user name. For curl-style `-u name:password` options only the
password part is redacted. The program name and argument layout are kept, so
callers can still tell what is running.

The same filtering covers `--creds`, escaped spaces in shell headers, spaced PowerShell header assignments, and Git `http.extraHeader` authorization values. If a process rewrites its program name into a full command line, credential-shaped arguments are filtered in `argv0`, `argv`, and `cmdline`; ordinary executable paths, including paths with spaces, stay unchanged.

For curl and curlie, explicit data and JSON options also filter credential-named form fields and nested JSON fields. Non-sensitive fields remain visible. Bodies larger than 64 KiB, JSON deeper than 16 levels, or malformed JSON are hidden as a whole; `@file` references are never read. Literal `@` values in `--data-raw` and `--form-string` are still filtered; multipart form and URL-encoded options keep each `name=value` intact rather than splitting `&` inside the value. Query and fragment parameters are filtered in absolute and relative URLs, including OAuth `code`; ordinary command options named `--code` are unchanged. This is diagnostic-output filtering, not a guarantee that arbitrary terminal text contains no secrets. Raw pane reads, layout exports and session restore data keep their existing behavior.
...
An agent's activity tree (subagents, tasks, todos, background processes) is a
server-owned fact. The server reads it from a per-agent source adapter on a
background thread; hooks only signal that something may have changed.

Adapters that read an agent's own session files (Claude Code, Codex, Kimi Code)
initially resolve the configuration directory from the Herdr server's environment:
`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, or `KIMI_CODE_HOME`, falling back to `~/.claude`,
`~/.codex`, or `~/.kimi-code`. Claude Code and Codex session hooks also report the
absolute transcript path with the session ID. Once received, that path takes
priority only while the pane still has the same agent and session ID, so a
`CLAUDE_CONFIG_DIR` or `CODEX_HOME` set only inside a pane works. Resume and fork
reports update this association; a new ID without a valid path falls back to the
server's configuration directory. Agent resume commands still use the session ID.
Kimi Code still needs its directory override in the server's environment.

Codex paths must name `rollout-<timestamp>-<session-id>.jsonl` or `.jsonl.zst` under
`sessions/YYYY/MM/DD/` or the flat `archived_sessions/` directory. Unknown layouts
are unsupported; Herdr does not guess a library from arbitrary parent directories.
Active copies take priority over archived copies; plain files take priority over
compressed copies of the same rollout. Both discovery and content reading support
read-only compressed rollouts, preserving the original JSONL byte cursor without
materializing or changing source files.

Compressed reads are limited to 16 MiB of input, 64 MiB of decoded content, an
8 MiB decoder window, and 128 frames per file. Each discovery/read call has a
64 MiB compressed-input and 72 MiB decoded-work budget (including unfinished
window reservations). The two workers share one cold decoder and a 128 MiB cache
with at most 2,048 entries; unchanged metadata, terminal status, and failures are
cached even after content eviction. File changes invalidate the cache. Exceeding
a file limit is unsupported; exhausted shared/work budgets are retried later, and
unreadable lifecycle data remains unknown. Discovery examines at most 8,192 date
directory entries and 8,192 file candidates, then 2,048 selected rollouts.

Codex 0.157 reverted threads may append `_<rollout-id>` after the stable session ID. Reports still match the stable ID; discovery and content reads select the newest filename timestamp, then rollout ID, and show each thread once. The activity reader renders the selected rollout; linked `history_base` prefixes from earlier rollouts are not expanded.

Codex cursors also carry the logical rollout identity. When a revert selects another rollout, `activity_cursor_stale` means discard accumulated content and restart with no cursor; the TUI does this on its next tick. Compression or movement into the archive preserves the identity. Legacy numeric cursor input remains accepted for compatibility, but cannot detect replacement; always return the opaque `next_cursor` from the server. Temporary file access failures are retried and are not cached as malformed data.


Hooks signal with `pane.report_agent_activity`:
...
```json
{
  ...
  "params": {
    ...
    "source": "herdr:pi",
    "agent": "pi",
    "hint": "{\"type\":\"herdr.activity.snapshot\",\"version\":1,\"nodes\":[]}"
  }
}
```

`hint` is optional. It can be a free-text marker such as the hook event name, or
a structured snapshot that the agent's source adapter understands: Herdr's Pi
extension reports the whole tree as a `herdr.activity.snapshot` JSON document
this way, because Pi keeps no activity on disk. The server keeps only the latest
hint per pane and hands it to the source adapter on the next refresh. The
whole request, hint included, must fit the API's 1 MiB request limit; a longer
request is dropped without a response and schedules nothing.
`node_id` and `seq` are accepted for diagnostics. The response is `ok` and never carries
the tree; apart from a snapshot hint like Pi's, the request does not carry it
either.

A hint schedules a refresh right away, rate limited to one refresh per pane per
second, plus one follow-up refresh about 2 seconds later (unless another refresh
runs first), because hooks often fire before the agent has written the change to
disk. Independently of hints,
the server refreshes an agent once when it is first detected, every 5 seconds
while its status is `working`, and once more when work ends (with the same
follow-up). While the stored tree still has a `pending`, `running`, or `blocked`
node whose `kind` is not `todo`, the server also keeps refreshing every 5
seconds after the agent goes idle, so a sub-agent that finishes after its
parent's turn still reaches its final status; this stops once those nodes end
or the tree has not changed for 10 minutes. Each refresh keeps at most 32 nodes
per agent (running nodes and their ancestors first) and records how large the
tree was before truncation.

`agent.get`, `agent.list`, and `session.snapshot` expose the stored tree as
`AgentInfo.activity`, omitted when empty. Each node carries `id`, `kind`
(`subagent`, `task`, `todo`, `background`, `unknown`), `label`, `status`
(`pending`, `running`, `blocked`, `done`, `failed`, `unknown`), and optional
`parent_id`, `agent_type`, `started_at_ms`, `ended_at_ms`, `content_ref`, and
`summary`. Unknown enum values decode as `unknown`. `AgentInfo.activity_running`,
`activity_total`, and `activity_truncated` describe the tree as the source
reported it, before truncation; each is omitted when zero or false, so all three
are absent when the agent has no activity. When `activity_truncated` is set, `activity` holds at most 32 nodes in source
order — running nodes with their ancestors are kept first, then other nodes whose
parent was kept — so fetch the whole tree with `agent.activity.read` without
`node_id`. `AgentInfo.launch_seq` is
the server-assigned launch order: a pane receives a new monotonic number when it
first gains an agent identity, keeps it across state changes, and gets a new one
after the agent is released or re-detected. `0` means unknown (older servers
omit the field); the number is only comparable within one server process.

Read a tree or one node's content with `agent.activity.read`:

```json
{
  "id": "req_5",
  "method": "agent.activity.read",
  "params": {
    "pane_id": "w1:p1",
    "node_id": "call_par/0",
    "cursor": null,
...
Give exactly one of `pane_id` or `external_id`, otherwise the call fails with
`invalid_params`. Without `node_id` the response carries the full tree in
`nodes` (for a `pane_id` target this read also refreshes the stored tree, so the
next snapshot matches it unless a refresh that started later is already stored.
Stored trees and lists are ordered by when their read
started, not by when they arrive: a background refresh of that pane that
started before this read is discarded when it lands later, and one that
started after it replaces it as usual. For an `external_id` target it refreshes
that source's list instead, where the entry can still hold only part of this
tree); with `node_id` it carries an empty `nodes` list and a `content` chunk:
`node_id`, `format` (`text`, `markdown`, `jsonl`, `unknown`), `text`, `eof`,
`truncated`, and `next_cursor` to pass back as `cursor` for the next page. `cursor` is opaque to the caller. `max_bytes`
defaults to 65536 and is capped at 524288. `follow: true` keeps the server
refreshing that pane's tree every second for the next 10 seconds so a viewer
that polls can track a running node; it does not move where a content read
starts, so keep paging with `cursor`, and `external_id` targets ignore it. The
response is
`{"type":"agent_activity","nodes":[...],"content":{...}}` and is produced on the
background reader thread, so it may arrive after responses to later requests.

Errors: `invalid_params` (besides the target rule above, an empty `node_id`,
`max_bytes: 0`, or a malformed `external_id`), `pane_not_found`,
`agent_not_found` (the pane hosts no agent, the external source is unknown, or,
for a tree read, that source no longer lists the external session), `not_implemented` (this server has no source adapter for that agent,
or the adapter cannot answer the request, for example because Herdr has not
received the agent's native session reference yet), `activity_unavailable` (the
source cannot be read right now, for example a node read before Pi has reported
a snapshot), `activity_malformed` (the source or the cursor is not in the expected
format), `activity_io_error`, `server_busy` when the reader queue is full, and
`server_unavailable` when the background reader is not running.

Herdr ships source adapters for Claude Code, Codex, Kimi Code, and OpenCode,
which read each agent's own local session data, and for Pi, which reads the
snapshot from Herdr's Pi extension. An agent without an adapter, such as one
reported under a custom name, answers `not_implemented`. Claude Code, Codex, and
OpenCode locate their session through the native session reference Herdr has
stored for the pane, normally reported by the Herdr integration when the session
starts, so without one their tree stays empty; Kimi Code falls back to the most
recently active session in the pane's working directory. The OpenCode adapter
runs the `opencode` command found on the server's `PATH` and follows the
server's `XDG_DATA_HOME`; without that command its reads fail with
`activity_io_error`.

`agent.external.list` (no params) lists agent sessions that belong to no pane,
as `{"type":"external_agent_list","agents":[...]}` with `ExternalAgentInfo`
entries: `external_id` (`<source>:<session id>`, usable as `external_id` in
`agent.activity.read`), `source`, `agent_status`, `label`, `readable` (`false` marks an
entry whose source has not been read successfully for 60 seconds), and
optional `agent`, `cwd`, `updated_at_ms`, and `activity`. The one external
source today is `zcode`, the ZCode desktop app: it lists up to 10 unarchived
root sessions updated in the last 72 hours, read-only from ZCode's local
database through the system `sqlite3` command, each with its subagent tree and
todos. Those 10 roots share a single 300-row cap for the whole query, so a large
early root can leave later roots showing only themselves in this list; reading
one root's full tree afterwards through `agent.activity.read`'s `external_id`
runs an independent query scoped to that root alone, with its own 300-row cap
that is not shared with the list. Without ZCode's database (ZCode not installed
or never started) the call
succeeds with an empty list; hide the external section of a client when the list
comes back empty. When `sqlite3` is missing or the database cannot be read, the
call fails with `activity_unavailable`. `not_implemented` is returned only when
every registered source reports that it cannot enumerate external sessions.

When a refresh changes a stored tree the server emits
`pane.agent_activity_changed` with `pane_id`, `running`, and `total`. The event
carries counts only; read the tree with `agent.get` or `agent.activity.read`.
Subscriptions to this event take no `pane_id` filter, so filter on the event's
`pane_id` yourself.
...
A burst of lifecycle events arrives as consecutive lines in a single delivery
pass instead of one line per polling interval. Delivery is still driven by a
polling loop, so the latency of the first line in a burst is bounded by the
polling interval. Snapshot-based subscriptions (`pane.output_matched`,
`pane.agent_status_changed`, `pane.scroll_changed`) are edge triggered and emit
at most one event per polling interval. Writing on a subscription connection
(for example a newline keepalive) does not end the stream; those bytes stay
buffered and unread until the connection closes.

Every lifecycle event line carries an optional `sequence`, the event's position
in this server's event sequence. Use it to reconcile ordering or to tell how far
a stream has advanced. Snapshot-based frames have no sequence and omit the
field, and older servers omit it everywhere.

The server keeps a bounded ring buffer of recent events. If a subscriber falls
behind far enough that events it subscribed to are evicted before delivery, the
stream can send a notice line before the events that survived:

```json
{"event":"events.lost","data":{"from":601,"to":688}}
```

Notice lines are opt in. Set `notices: true` in the `events.subscribe` params to
receive them; the field defaults to `false`, so a subscription that omits it sees
exactly the lines it saw before. A gap is only reported when an event kind this
subscription asked for was evicted — unrelated traffic rolling through the buffer
does not trigger one — and a connection receives at most one notice per polling
round even when several of its subscriptions were affected.

`from` and `to` are the inclusive range of sequence numbers in which events you
subscribed to were dropped and cannot be recovered. Treat it as a resync signal:
re-read `session.snapshot` (or the state you cache) and then keep applying the
stream.

The notice is a new line type, not a change to existing event frames. This makes
the contract for the `events.subscribe` stream explicit: a line is either a
`stream_event` (an event frame with an optional `sequence`) or an
`event_stream_notice`, and clients **must** ignore any `event` name they do not
recognize rather than treating it as an error. `events.lost` is not an event
kind, so a client that decodes every line strictly against the `event` schema
entry has to opt out of notices (the default) or decode against `stream_event`
and `event_stream_notice` together.
...
`pane.moved`, `pane.exited`, `pane.agent_detected`, `pane.agent_activity_changed`,
...
Observation data uses dedicated methods instead of `events.subscribe`: `account.usage.subscribe` takes the same params as `account.usage.get`, answers with a `subscription_id`, and then pushes `account.usage.updated` (changed account snapshots plus their `refresh` states) and `account.usage.refreshing` (refresh states only, as a probe starts) to that connection until `account.usage.unsubscribe`; `system.metrics.subscribe` pushes `system.metrics.updated` the same way. Subscribers keep the server probing in the background, so unsubscribe when the data is no longer shown. Subscriptions die with the connection and must be created again after a reconnect. The generation-1 client endpoint receives these pushes as `endpoint.observation.v1` control frames carrying the server `boot_id` and the event; unknown event kinds are ignored rather than rejected.

Observation pushes use the same `{"event": "<name>", "data": {...}}` envelope as `events.subscribe`. `account.usage.updated` carries `accounts` (the changed `AccountUsageSnapshot` entries) and an optional `refresh` list; `account.usage.refreshing` carries only `refresh`; `system.metrics.updated` carries `snapshot`. A `refresh` entry (`UsageRefreshState`) is also returned by `account.usage.get` and `account.usage.refresh`, aligned to `accounts` by `account_id`; older servers omit the list. Its fields: `in_flight` and `queued` (a probe has been dispatched / is still waiting for the probe thread); `requested_at_ms` and `attempted_at_ms` (the last explicit refresh, including debounced ones, and the last probe actually started); `next_allowed_at_ms` (the earliest time an explicit refresh is accepted again — absent means now); `retry_after_ms` (a vendor backoff such as HTTP 429, which explicit refreshes never bypass); `binding_inferred` (the request named an unbound pane and the data came from the vendor's only configured account without writing a binding); `trust_required` (the interactive probe stopped at the CLI's folder-trust dialog; the snapshot `message` names the directory); `callback_only` (samples only arrive through a push — the official callback, or Herdr's own integration extension for Pi); `pending_binding` (`pane_id`, `agent`, `candidates`, `rejected_at_ms` after a callback from an unbound pane was rejected); and `callback_enabled` (whether the official callback is wired into herdr, absent when unknown or when the vendor has no callback, such as Pi). `binding_inferred` and `pending_binding` are request-scoped and stay at their defaults in events; read them from the `account.usage.get` response.
...
```json
{
  ...
  "result": {
    ...
    "manifests": [
      {
        "agent": "codex",
        "source": "/home/me/.config/herdr/agent-detection/codex.toml",
...
ignore unknown fields and handle unsupported methods as normal errors. On pushed
streams they must also ignore lines whose `event` name they do not recognize:
new line types such as `events.lost` are added there, and opting in to them is
what `events.subscribe`'s `notices` flag controls.

Additions inside the generation-1 endpoint stay optional too: the client may
report its host `ssh_auth_sock` in the handshake so a detached server can fall
back to it once the host terminal restarts, and the server pushes
`endpoint.terminal-cwd.v1` control frames when the focused pane's directory
changes, so the client can write the host terminal's OSC 7. Unknown control
frame kinds, unknown handshake fields, and unknown event names are ignored
rather than rejected.

## Immutable text snapshots

Servers advertise `pane.text_snapshot.capture`, `pane.text_snapshot.read`, `pane.text_snapshot.selection`, `pane.text_snapshot.retain`, and `pane.text_snapshot.release` independently of the generation 1 binary codec. Capture takes `pane_id` and returns a server boot identifier, a snapshot identifier, and an initial styled window with absolute row coordinates. The captured range and truncation flag describe retained history; read takes `snapshot_id`, `start_row`, and `rows`. Selection takes the identifier plus inclusive `anchor` and `cursor` points. It never re-reads live terminal content.

Client-shell tokens belong to their connection. Retain acknowledges installation of the snapshot; unacknowledged captures expire in 60 seconds, while acknowledged snapshots are released with the client or its surface. Raw JSON API snapshots expire after five minutes without use. Release is idempotent. Captures have per-snapshot and global bounds; callers must handle `snapshot_capacity`, `snapshot_expired`, `snapshot_range`, and `snapshot_unavailable` without silently copying live content.

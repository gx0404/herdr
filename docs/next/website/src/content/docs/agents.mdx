...
Startup prompts that wait for a decision before the agent session begins count as blocking UI too, for example the Codex "Hooks need review" chooser and the Kimi Code CLI "Trust this folder?" prompt. While such a prompt is open the pane is `blocked`, and `herdr agent start` reports that the agent is not ready instead of succeeding.

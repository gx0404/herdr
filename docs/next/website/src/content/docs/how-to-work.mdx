...
The same persistent Herdr session opens in your phone terminal. When the screen is smaller than the docked layout's minimum size, Herdr switches to a compact view that shows one panel at a time, so you can inspect agents, switch workspaces, and check panes without leaving SSH. The compact view's title bar lists the panels; click a name to switch to that panel, or, when the names do not fit, use **Arrange layout** and `Tab`. See [Mouse UI](/docs/concepts/#mouse-ui) for the minimum sizes. The mobile switcher and `ui.mobile_width_threshold` belong to the classic layout, which Herdr uses only when the server is too old for the docked layout.
...
<div class="mobile-doc-shots">
  ...
  <figure>
    <img src="/assets/mobile-switch-menu-v2.jpeg" alt="Herdr mobile switcher of the classic layout on a phone" loading="lazy" />
    <figcaption>mobile switcher (classic layout, older servers)</figcaption>
  </figure>
</div>

...
In a detected Codex pane, an unmodified left-button drag uses Herdr’s frozen text selection and copies on release when `ui.copy_on_select` is enabled. A click still goes to Codex; hold Alt throughout a gesture to send it directly to Codex. This requires the connected server’s text snapshot support. Right-click the same pane and choose **Copy** to copy the completed selection, including just after automatic copying. Automatic copying resumes live output and temporarily retains only the selected text for this menu action. New input or selection, scrolling, changing panes, resizing, and reconnecting clear that retained text. The pane menu disables **Copy** when there is no current or retained selection; it never sends an interrupt key to the application. If right-click passthrough is enabled for that pane, use the selection copy shortcut instead.

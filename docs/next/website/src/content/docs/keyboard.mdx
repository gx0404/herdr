...
Overlay pages carry their own local keys, shown in each page's footer: on the machines page (`prefix+m`), `b` opens the broadcast target set and a machine's detail card offers `o` for the remote file browser. While input broadcast is on, the mode bar shows a permanent `⇄ BROADCAST ×N` badge so you can never miss that your typing fans out to other machines.

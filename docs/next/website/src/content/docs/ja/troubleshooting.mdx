...
## 幅の変更後に全角文字の隣の新しい文字が消える

ペインを狭めてから広げた後も、切り取られた CJK 文字や絵文字の隣に新しく出力された文字は表示されます。元の全角文字の 2 列目に書き込むと、完全な全角文字の右半分を上書きした場合と同様に古い文字が消去されます。上書きせずに広げた場合は、切り取られた文字が復元されます。古いビルドで新しい文字が消える場合は、Herdr を更新し、該当する server を再起動してください。

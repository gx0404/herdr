...
同じライブラリが UI も駆動します。コマンドパレットの「スニペット」「スニペットを実行…」でスニペットオーバーレイを開くと、ライブラリの管理（新規/編集/削除）と、ターゲット選択（現在のペイン、オンラインのマシンの任意のペイン、または選択した各マシンのフォーカス中ペイン）→宣言済み変数の入力→レンダリング結果の確認という実行フローを辿れます。実行は接続済みエンドポイントのセッション経由で送られ（追加の SSH ブリッジは不要）、ターゲットごとの結果はトーストに集約され（すべてのターゲットが受け付けた場合は ✓ 成功スタイル）、同じ履歴に書き込まれます（オーバーレイの履歴ページで確認できます）。

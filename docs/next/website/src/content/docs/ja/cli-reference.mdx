...
同じ集合が UI 内のブロードキャストも駆動します。マシンページで `b`、またはコマンドパレットの「broadcast input…」から開き、`g` で切り替え、`a` でオンラインマシンからペインを追加します。ブロードキャスト中は mode bar に `⇄ ブロードキャスト ×N` バッジが常駐し、フォーカス中のペインへの入力はすべて登録ペインに扇出されます（そのペイン自身は除外。マウス入力は扇出されません）。詳しくは[マシンへの接続](/ja/docs/connecting-machines/#入力ブロードキャスト)を参照してください。

...
`pane.process_info` は、ペインのシェル pid、利用可能な場合はフォアグラウンドプロセスグループ id、そしてプラットフォームが公開している場合は pid、名前、argv/cmdline、cwd を含むフォアグラウンドプロセスを返します。argv/cmdline のうち資格情報に見える値は `[REDACTED]` に置き換えられます。対象は `--token` や `--api-key` などのオプションの値、`Authorization:` などの機密ヘッダー(`Bearer`/`Basic` などの認証スキーム名は残ります)、`Bearer` トークン、`*_KEY=` や `*_TOKEN=` の代入、URL 内のパスワードと資格情報系のクエリパラメーターです。プログラム名と引数の構成はそのまま残るため、呼び出し側は何が実行中かを引き続き判別できます。

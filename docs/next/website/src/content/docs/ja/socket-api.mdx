...
通常の `herdr api schema` は短い概要を表示します。`--json` はツール向けの完全な JSON Schema を表示し、`--output PATH` はそのドキュメントをファイルに書き込みます。スキーマには、生のリクエスト、成功レスポンス、エラーレスポンス、発行イベント、購読イベントが含まれます。`events.subscribe` ストリームに push される行は、`stream_event` エントリ(イベントフレームと省略可能な `sequence`)と `event_stream_notice` エントリ(`events.lost` などストリームのメタ情報)が記述します。`event` エントリが記述するのはイベントのペイロード自体です。
...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear`、`agent.activity.read`、`agent.external.list` |
...
| 観測 | `system.metrics.get`、`system.metrics.subscribe`、`system.metrics.unsubscribe`、`system.process.list`、`system.process.get`、`system.process.terminate`、`account.usage.providers`、`account.usage.get`、`account.usage.refresh`、`account.usage.subscribe`、`account.usage.unsubscribe`、`account.usage.report`、`account.usage.integration`、`account.binding.set` |
...
`pane.close` はペインを切り離した時点で応答します。ペイン内のプロセスは SIGHUP、SIGTERM、SIGKILL の順にバックグラウンドで終了させるため、終了しないプロセスがあっても同じ接続や他の接続のリクエストが止まりません。これらのプロセスの終了を前提とする呼び出し側は、応答が返った時点で終了したとみなさず、ポーリングして確認してください(例えば残っているペインへの `pane.process_info`、または独自のプロセス確認)。
...
`pane.process_info` は、ペインのシェル pid、利用可能な場合はフォアグラウンドプロセスグループ id、そしてプラットフォームが公開している場合は pid、名前、argv/cmdline、cwd を含むフォアグラウンドプロセスを返します。argv/cmdline のうち資格情報に見える値は `[REDACTED]` に置き換えられます。対象は `--token` や `--api-key` などのオプションの値、`Authorization:` などの機密ヘッダー(`Bearer`/`Basic` などの認証スキーム名は残ります)、`Bearer` トークン、`*_KEY=` や `*_TOKEN=` の代入、URL 内のパスワードと資格情報系のクエリパラメーターです。http(s) の URL でパスワードを含まないユーザー情報は多くの場合トークンなので、丸ごと伏せられます(`https://user@bitbucket.org/…` は `https://[REDACTED]@bitbucket.org/…` になります)。`ssh://git@…` など他のスキームではユーザー名が残ります。curl の `-u 名前:パスワード` のような指定ではパスワード部分だけが伏せられます。プログラム名と引数の構成はそのまま残るため、呼び出し側は何が実行中かを引き続き判別できます。

同じフィルターは `--creds`、shell ヘッダー内のエスケープされた空白、空白を含む PowerShell ヘッダー代入、Git の `http.extraHeader` の認証値にも適用されます。プロセスがプログラム名をコマンド行全体に書き換えた場合、`argv0`、`argv`、`cmdline` の資格情報に見える引数も伏せられます。空白を含む通常の実行ファイルのパスは変更しません。

curl と curlie の明示的な data / JSON オプションでは、フォームと入れ子の JSON にある資格情報名のフィールドも伏せ、その他のフィールドは残します。本文が 64 KiB を超える場合、JSON が 16 階層を超える場合、または JSON が不完全な場合は本文全体を伏せます。`@file` の参照先は読み取りません。`--data-raw` と `--form-string` のリテラルの `@` 値もフィルター対象です。multipart フォームと URL エンコードのオプションは 1 つの `name=value` として扱い、値の中の `&` で分割しません。絶対 URL と相対 URL のクエリおよび fragment のパラメーターも対象となり、OAuth の `code` を含みますが、通常の `--code` オプションは変更しません。これは診断出力のフィルターであり、任意の端末本文に秘密がないことを保証するものではありません。ペインの原文読み取り、レイアウトのエクスポート、セッション復元データは従来の動作を維持します。
...
Worktree メソッドは Git チェックアウトを Herdr ワークスペースとして管理します。`worktree.create` はチェックアウトを作成し、新しい `workspace`、`tab`、`root_pane`、`worktree` のレコードを返します。要求されたブランチがローカルに既存ならそれをチェックアウトし、なければ要求されたベースまたは `HEAD` からブランチを作成します。`worktree.open` は既存のチェックアウトを開くか、すでに開いているワークスペースを返します。`worktree.remove` はリンクされた子ワークスペースに対して `git worktree remove` を実行し、ブランチは決して削除しません。`worktree.list` の結果にある `is_prunable` は、フォルダーが失われても Git にまだ登録されているリンク済みチェックアウトを示します。Herdr はどの Git バージョンでもチェックアウトの状態から導出し、Git 2.31 以降が出力する `prunable` 行だけには頼りません。
...
エージェントのアクティビティツリー(サブエージェント、タスク、Todo、バックグラウンド
プロセス)はサーバーが所有する事実です。サーバーはエージェントごとのソースアダプターを
通じてバックグラウンドスレッドでそれを読み取り、フックは「変わったかもしれない」という
合図を送るだけです。

エージェント自身のセッションファイルを読むアダプター(Claude Code、Codex、Kimi Code)は、
最初に Herdr サーバーの環境にある `CLAUDE_CONFIG_DIR`、`CODEX_HOME`、
`KIMI_CODE_HOME` を使い、なければ `~/.claude`、`~/.codex`、`~/.kimi-code` を使います。
Claude Code と Codex のセッションフックは、セッション ID と絶対トランスクリプトパスも
報告します。ペインの現在のエージェントとセッション ID が一致する間はそのパスが優先され、
ペイン内だけの `CLAUDE_CONFIG_DIR` や `CODEX_HOME` でも動作します。resume・fork の
報告で対応関係を更新し、新しい ID に有効なパスがなければサーバーの設定ディレクトリに
フォールバックします。再開コマンドは引き続きセッション ID を使います。Kimi Code は
引き続きサーバー側の環境にディレクトリの上書きが必要です。

Codex のファイル名は `rollout-<timestamp>-<session-id>.jsonl` または `.jsonl.zst`、
配置は `sessions/YYYY/MM/DD/` または平坦な `archived_sessions/` です。未知の配置は
非対応で、任意の親ディレクトリからライブラリを推測しません。アクティブ側をアーカイブ
より優先し、同じ rollout の通常ファイルを圧縮コピーより優先します。検出と内容の
読み取りは圧縮ファイルにも対応し、元の JSONL バイトカーソルを保持します。
展開したファイルの保存や元ファイルの変更は行いません。

圧縮ファイルごとの上限は入力 16 MiB、展開内容 64 MiB、デコードウィンドウ 8 MiB、
128 フレームです。検出/読み取り一回の予算は圧縮入力 64 MiB、デコード処理 72 MiB
（未完了フレームのウィンドウ予約を含む）です。二つの worker は一つのコールド
デコーダーと最大 2,048 件・128 MiB のキャッシュを共有します。本文の追い出し後も
変更のないメタデータ、終了状態、失敗結果を保持し、ファイル変更時に無効化します。
ファイル上限超過は非対応、共有/処理予算不足は後で再試行、読めない状態は不明です。
検出は最大 8,192 個の日時ディレクトリ項目と 8,192 個の候補ファイルを調べ、
そのうち最大 2,048 個の選択された rollout を読み取ります。

Codex 0.157 の revert 後のスレッドでは、安定したセッション ID の後に `_<rollout-id>` が付く場合があります。報告は安定 ID と照合し、検出と読み取りはファイル名の時刻、rollout ID の順で最新版を選び、各スレッドを一度だけ表示します。選択された rollout を表示し、以前の rollout を参照する `history_base` の履歴プレフィックスは展開しません。

Codex カーソルには論理 rollout の識別情報も含まれます。revert で別の rollout が選ばれた場合、`activity_cursor_stale` は蓄積した内容を破棄してカーソルなしで再読することを示し、TUI は次の tick で実行します。圧縮やアーカイブへの移動では識別情報が変わりません。互換性のため旧数値カーソルも受け付けますが、置換は検出できません。サーバーの不透明な `next_cursor` をそのまま返してください。一時的なファイルアクセス失敗は再試行し、不正形式としてキャッシュしません。


フックは `pane.report_agent_activity` で合図します:
...
`hint` は省略可能です。フックのイベント名のような自由テキストでも、そのエージェントの
ソースアダプターが理解できる構造化スナップショットでもかまいません。Pi はディスク上に
アクティビティを残さないため、Herdr の Pi 拡張はツリー全体を `herdr.activity.snapshot`
JSON ドキュメントとしてこの方法で報告します。サーバーはペインごとに最新の hint を 1 つ
だけ保持し、次回の更新時にソースアダプターへ渡します。hint を含むリクエスト全体は
API の 1 MiB のリクエスト上限に収まる必要があり、それを超えるリクエストは応答なしで
破棄され、更新もスケジュールされません。`node_id` と
`seq` は診断用に受理されます。レスポンスは `ok` で、ツリーを含むことはありません。Pi のようなスナップショット
hint を除き、リクエストにもツリーは含まれません。

hint はすぐに更新をスケジュールしますが、同じペインの更新は毎秒 1 回に制限されます。
フックはエージェントが変更をディスクへ書き込む前に発火することが多いため、約 2 秒後に
もう 1 回更新します(それまでに別の更新が走った場合は行いません)。hint とは独立に、サーバーはエージェントを最初に検出したときに
1 回、状態が `working` の間は 5 秒ごとに、作業が終わったときにもう 1 回(同じく約
2 秒後にさらに 1 回)更新します。保存されたツリーに `kind` が `todo` 以外で状態が
`pending`、`running`、`blocked` のノードが残っている間は、エージェントがアイドルに
なった後も 5 秒ごとに更新を続けるため、親のターンより後に終わったサブエージェントも
最終状態に到達します。これらのノードがすべて終わるか、ツリーが 10 分間変化しなければ
停止します。各更新はエージェントごとに最大 32 ノード(実行中のノードとその祖先を
優先)を保持し、切り詰める前のツリーの大きさを記録します。

`agent.get`、`agent.list`、`session.snapshot` は保存されたツリーを `AgentInfo.activity`
として公開します(空なら省略)。各ノードは `id`、`kind`(`subagent`、`task`、`todo`、
`background`、`unknown`)、`label`、`status`(`pending`、`running`、`blocked`、`done`、
`failed`、`unknown`)と、任意の `parent_id`、`agent_type`、`started_at_ms`、
`ended_at_ms`、`content_ref`、`summary` を持ちます。未知の列挙値は `unknown` として
デコードされます。`AgentInfo.activity_running`、`activity_total`、
`activity_truncated` は、切り詰める前にソースが報告したツリーを表します。
それぞれ 0 または false のときに省略されるため、エージェントにアクティビティが
なければ 3 つとも現れません。
`activity_truncated` が立っているとき `activity` にはソースの順で最大 32 ノードしか
ありません(実行中のノードとその祖先を先に残し、次に親が残ったほかのノードを
補います)。ツリー全体は `node_id` なしの `agent.activity.read` で取得してください。
`AgentInfo.launch_seq` はサーバーが割り当てる起動順です。ペインが
最初にエージェントの身元を得たときに新しい単調増加の番号を受け取り、状態が変わっても
保持し、エージェントが解放または再検出されると新しい番号になります。`0` は不明を意味し
(古いサーバーはこのフィールドを省略します)、番号は同じサーバープロセス内でのみ比較
できます。

ツリーまたは 1 つのノードの内容は `agent.activity.read` で読み取ります:
...
`pane_id` と `external_id` はちょうど一方だけを指定します。そうでなければ呼び出しは
`invalid_params` で失敗します。`node_id` を省くとレスポンスは `nodes` にツリー全体を
含みます(対象が `pane_id` のとき、この読み取りは保存されたツリーも更新するため、次の
スナップショットはこれと一致します(より後に開始した更新の結果がすでに保存されている場合を除きます)。保存されるツリーと一覧は、届いた順ではなく読み取りを開始した順で扱われます。
この読み取りより先に開始したそのペインのバックグラウンド更新は、後から届いても破棄され、後に開始した更新は通常どおりこれを置き換えます。
対象が `external_id` のときに更新されるのはそのソースの一覧で、一覧上のその項目は
このツリーの一部しか持たないことがあります)。`node_id` を指定すると `nodes` は空になり、レスポンスは `content` チャンクを
含みます:`node_id`、`format`(`text`、`markdown`、`jsonl`、`unknown`)、`text`、`eof`、
`truncated`、そして次のページで `cursor` として返す `next_cursor` です。`cursor` は呼び出し側にとって不透明です。
`max_bytes` の既定値は 65536 で、上限は 524288 です。`follow: true` にすると、サーバーは
その後 10 秒間、そのペインのツリーを毎秒更新するため、ポーリングするビューアーは
実行中のノードを追跡できます。内容読み取りの開始位置は変わらないので続きは
`cursor` で辿ってください。`external_id` の対象はこれを無視します。レスポンスは
`{"type":"agent_activity","nodes":[...],"content":{...}}` の形で、バックグラウンドの
読み取りスレッドが生成するため、後続のリクエストへのレスポンスより遅れて届くことが
あります。

エラー:`invalid_params`(上記の対象の規則のほか、`node_id` が空、`max_bytes: 0`、
`external_id` の形式が不正)、`pane_not_found`、`agent_not_found`(ペインにエージェントが
いない、外部ソースが未知、またはツリー全体の読み取りでそのソースがその外部セッションを
もう列挙していない)、`not_implemented`(このサーバーにそのエージェントのソースアダプターが
ない、またはアダプターがこのリクエストに応答できない。たとえば Herdr がまだその
エージェントのネイティブセッション参照を受け取っていない)、`activity_unavailable`(ソースを
今は読めない。たとえば Pi がまだスナップショットを報告していないときのノードの読み取り)、`activity_malformed`
(ソースの内容またはカーソルが期待した形式ではない)、`activity_io_error`、読み取り
キューが満杯のときの `server_busy`、バックグラウンドの読み取りスレッドが動いていないときの
`server_unavailable`。

Herdr は Claude Code、Codex、Kimi Code、OpenCode 向けに各エージェント自身のローカル
セッションデータを読むソースアダプターを、Pi 向けに Herdr の Pi 拡張が報告する
スナップショットを読むアダプターを同梱しています。アダプターのないエージェント
(カスタム名で報告されたものなど)は `not_implemented` を返します。Claude Code、Codex、
OpenCode は Herdr がそのペインに保存したネイティブセッション参照(通常はセッション開始時に
Herdr インテグレーションが報告します)でセッションを特定するため、参照がなければツリーは
空のままです。Kimi Code は代わりにペインの作業ディレクトリで最近アクティブだった
セッションを使います。OpenCode のアダプターはサーバーの `PATH` にある `opencode`
コマンドを実行し、サーバー環境の `XDG_DATA_HOME` に従います。そのコマンドがなければ
読み取りは `activity_io_error` で失敗します。

`agent.external.list`(パラメーターなし)は、どのペインにも属さないエージェント
セッションを `{"type":"external_agent_list","agents":[...]}` として列挙します。エントリーは
`ExternalAgentInfo` で、`external_id`(`<source>:<セッション id>`。`agent.activity.read` の
`external_id` として使えます)、`source`、`agent_status`、`label`、`readable`(`false` はそのエントリーのソースを 60 秒間
正常に読めていないことを示します)と、任意の
`agent`、`cwd`、`updated_at_ms`、`activity` を持ちます。現在の外部ソースは ZCode
デスクトップアプリの `zcode` だけです。システムの `sqlite3` コマンドで ZCode のローカル
データベースを読み取り専用で読み、直近 72 時間に更新されたアーカイブされていない
ルートセッションを最大 10 件、それぞれのサブエージェントツリーと Todo 付きで列挙します。
この 10 件のルートは 1 回のクエリで 300 行の上限を共有するため、先に展開される
大きなルートがあると、後続のルートは一覧でそのルート自身しか残らないことが
あります。あるルートの `external_id` を使って `agent.activity.read` で改めて
ツリー全体を読むときは、そのルートだけを対象にした独立したクエリになり、一覧の
クエリとは共有しない専用の 300 行上限を持ちます。
ZCode のデータベースがない(ZCode が未インストール、または一度も起動していない)場合、
この呼び出しは空のリストで成功します。リストが空で返ってきたらクライアントは外部
エントリーの領域を隠してください。`sqlite3` がない、またはデータベースを読めない場合は
`activity_unavailable` で失敗します。登録済みのソースがすべて外部セッションを列挙できない
と報告したときだけ `not_implemented` になります。

更新によって保存されたツリーが変わると、サーバーは `pane_id`、`running`、`total` を持つ
`pane.agent_activity_changed` を発行します。このイベントはカウントだけを運びます。
ツリーは `agent.get` または `agent.activity.read` で読み取ってください。このイベントの
購読は `pane_id` による絞り込みを受け付けないため、イベントの `pane_id` で自分で
絞り込んでください。
...
最初のレスポンスは購読の確認応答です。以降の行はプッシュされるイベントです。ライフサイクルイベントの購読はリクエストが受理された時点から始まり、それ以前に保持されていたイベントは再生しません。まとまって発生したライフサイクルイベントは、ポーリング間隔ごとに1行ではなく、1回の配信でまとめて連続した複数行として届きます。配信は引き続きポーリングループが駆動するため、まとまったイベントの最初の1行の遅延はポーリング間隔が上限です。スナップショットに基づく購読(`pane.output_matched`、`pane.agent_status_changed`、`pane.scroll_changed`)はエッジトリガーで、1ポーリング間隔あたり最大1件のイベントしか出力しません。購読接続に書き込んでも(改行のキープアライブなど)ストリームは終了しません。そのバイト列は接続が閉じるまでバッファに残り、消費されません。

ライフサイクルイベントの各行には、この server のイベント列における位置を表す省略可能な `sequence` が付きます。順序の突き合わせや、ストリームがどこまで進んだかの判断に使えます。スナップショットに基づくフレームにはシーケンス番号がないためフィールドを省略し、古い server はどこでも省略します。

server が保持する直近イベントのリングバッファには上限があります。購読者が遅れ、**その購読が対象とするイベント**が配信前に押し出された場合、ストリームは生き残ったイベントの前に通知行を送ることができます:

```json
{"event":"events.lost","data":{"from":601,"to":688}}
```

通知行はオプトインです。受け取るには `events.subscribe` のパラメータで `notices: true` を指定します。既定値は `false` なので、これを省略した購読ではこれまでとまったく同じ行だけが届きます。ギャップは、その購読が指定したイベント種別が実際に押し出されたときにだけ報告され、無関係なトラフィックがバッファを流れただけでは発生しません。また、同じ接続で複数の購読が影響を受けても、1 回のポーリングにつき通知は最大 1 行です。

`from` と `to` は、あなたが購読していたイベントが破棄され復元できないシーケンス番号の閉区間です。これは再同期の合図として扱ってください: `session.snapshot`(または自前でキャッシュしている状態)を読み直してから、ストリームの適用を続けます。

この通知は新しい行の種別であり、既存のイベントフレームの形状を変更しません。これにより `events.subscribe` ストリームの契約が明示されます: 各行は `stream_event`(省略可能な `sequence` を持つイベントフレーム)か `event_stream_notice` のいずれかであり、クライアントは認識できない `event` 名をエラーにせず**必ず**無視しなければなりません。`events.lost` はイベント種別ではないため、`event` エントリに対して各行を厳密にデコードするクライアントは、通知をオプトインしない(既定)か、`stream_event` と `event_stream_notice` の両方でデコードする必要があります。
...
観測データは `events.subscribe` ではなく専用メソッドを使います。`account.usage.subscribe` は `account.usage.get` と同じパラメーターを取り、`subscription_id` を返した後、その接続に `account.usage.updated`（変化したアカウントスナップショットとその `refresh` 状態）と `account.usage.refreshing`（プローブ開始時の更新状態のみ）を `account.usage.unsubscribe` までプッシュします。`system.metrics.subscribe` も同様に `system.metrics.updated` をプッシュします。購読者がいる間は server がバックグラウンドでプローブを続けるため、データを表示しなくなったら購読を解除してください。購読は接続とともに終了し、再接続後は作り直す必要があります。generation-1 のクライアントエンドポイントはこれらのプッシュを、server の `boot_id` とイベントを含む `endpoint.observation.v1` 制御フレームとして受け取り、未知のイベント種別は拒否せず無視します。

観測のプッシュは `events.subscribe` と同じ `{"event": "<名前>", "data": {...}}` エンベロープを使います。`account.usage.updated` は `accounts`（変化した `AccountUsageSnapshot` エントリ）と省略可能な `refresh` リストを、`account.usage.refreshing` は `refresh` だけを、`system.metrics.updated` は `snapshot` を運びます。`refresh` エントリ（`UsageRefreshState`）は `account.usage.get` と `account.usage.refresh` の応答にも含まれ、`account_id` で `accounts` と対応します。旧 server はこのリストを省略します。フィールドの意味: `in_flight` と `queued`（プローブが派遣済み / まだプローブスレッド待ち）、`requested_at_ms` と `attempted_at_ms`（デバウンスされたものを含む最後の明示的な更新と、実際に開始された最後のプローブ）、`next_allowed_at_ms`（次の明示的な更新が受け付けられる最も早い時刻。省略時は今すぐ可）、`retry_after_ms`（HTTP 429 などのベンダーバックオフ。明示的な更新でも回避しません）、`binding_inferred`（リクエストが未関連付けのペインを指定し、そのベンダーの唯一の設定済みアカウントからデータを返したが、関連付けは書き込んでいない）、`trust_required`（対話式プローブが CLI のディレクトリ信頼ダイアログで停止し、スナップショットの `message` にディレクトリが入る）、`callback_only`（サンプルはプッシュでのみ届く。公式コールバック、または Pi 向けの Herdr 統合拡張）、`pending_binding`（未関連付けペインのコールバックが拒否された後の `pane_id`、`agent`、`candidates`、`rejected_at_ms`）、`callback_enabled`（公式コールバックが herdr に接続済みか。不明な場合や、Pi のようにベンダーにコールバックがない場合は省略）。`binding_inferred` と `pending_binding` はリクエスト単位で埋められ、イベントでは既定値のままなので、`account.usage.get` の応答から読み取ってください。
...
番号付きバイナリプロトコルは、直接ターミナル接続やライブハンドオフなど、同一インストール内の内部操作に残ります。異なるビルド間でそれらを使う前に `ping` または `herdr status` を確認してください。JSON API クライアントは未知のフィールドを無視し、未対応メソッドを通常のエラーとして扱う必要があります。push されるストリームでは、認識できない `event` 名の行も無視する必要があります: `events.lost` のような新しい行の種別はそこに追加され、受け取るかどうかは `events.subscribe` の `notices` フラグで制御します。

generation-1 エンドポイント内の追加も同様に任意です。クライアントはハンドシェイクでホストの `ssh_auth_sock` を報告でき、detached server はホストターミナルの再起動後にそれをフォールバックとして使えます。フォーカス中のペインのディレクトリが変わると、server は `endpoint.terminal-cwd.v1` 制御フレームを push し、クライアントがホストターミナルの OSC 7 を書き込みます。未知の制御フレーム種別、未知のハンドシェイクフィールド、未知のイベント名は拒否せず無視します。

## 不変テキストスナップショット

サーバーは `pane.text_snapshot.capture`、`pane.text_snapshot.read`、`pane.text_snapshot.selection`、`pane.text_snapshot.retain`、`pane.text_snapshot.release` を個別に公開し、generation 1 のバイナリ codec は変更しません。capture は `pane_id` を受け取り、boot、スナップショット ID、スタイル付きの初期ウィンドウと絶対行座標を返します。保持範囲と切り捨てフラグが履歴の境界です。read は `snapshot_id`、`start_row`、`rows`、selection は ID と両端を含む `anchor`、`cursor` を受け取ります。選択はライブ端末を再読しません。

TUI のスナップショットは作成した接続に所属します。retain はクライアントへの取込を確認し、未確認の取得は60秒で失効します。確認済みは接続または表面の終了で解放します。JSON API は5分間使わないと失効し、release は繰り返せます。容量と `snapshot_capacity`、`snapshot_expired`、`snapshot_range`、`snapshot_unavailable` を処理し、ライブ内容のコピーに黙って切り替えないでください。

...
含みます(この読み取りは保存されたツリーも更新するため、次のスナップショットはこれと
一致します。読み取りが返った時点でそのペインでまだ実行中のバックグラウンド更新は、これを上書きせずに破棄され、その後もう一度更新されます)。`node_id` を指定すると `nodes` は空になり、レスポンスは `content` チャンクを

...
最大 32 ノード(実行中のノードとその祖先を優先)を保持し、ソースがそれ以上を報告した
場合は `truncated` を立てます。
...
デコードされます。`AgentInfo.launch_seq` はサーバーが割り当てる起動順です。ペインが
...
`agent`、`cwd`、`updated_at_ms`、`activity` を持ちます。外部ソースはまだ実装されていない
ため、このメソッドは現在 `not_implemented` を返します。

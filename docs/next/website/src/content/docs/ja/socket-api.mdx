...
だけ保持し、次回の更新時にソースアダプターへ渡します。`node_id` と `seq` は診断用に
受理されます。レスポンスは `ok` で、ツリーを含むことはありません。Pi のようなスナップショット
...
`activity_truncated` が立っているとき `activity` には実行中のノードとその祖先しか
残らないため、残りは `node_id` なしの `agent.activity.read` で取得してください。
...
ない)、`activity_unavailable`(ソースを今は読めない。たとえば Pi がまだスナップショットを
報告していない)、`activity_malformed`(ソースの内容またはカーソルが期待した形式では
ない)、`activity_io_error`、読み取りキューが満杯のときの `server_busy`。現在ツリーを提供
するのは Herdr の Pi 拡張を通じた Pi だけで、他のエージェントはそれぞれのソース
アダプターが実装されるまで `not_implemented` を返します。
...
`agent`、`cwd`、`updated_at_ms`、`activity` を持ちます。外部ソースのアダプターは
まだ 1 つもないため、この呼び出しは空のリストで成功します。リストが空で返って
きたらクライアントは外部エントリーの領域を隠してください。登録済みのソースが
すべて外部セッションを列挙できないと報告したときだけ `not_implemented` になります。

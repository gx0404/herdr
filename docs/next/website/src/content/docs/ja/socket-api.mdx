...
だけ(1 MiB まで)保持し、次回の更新時にソースアダプターへ渡します。これより大きい
hint も更新をスケジュールしますが、保持されるのは前の hint のままです。`node_id` と
...
もう 1 回更新します。hint とは独立に、サーバーはエージェントを最初に検出したときに
...
エージェントにアクティビティがなければ 3 つとも省略されます。
`activity_truncated` が立っているとき `activity` には保持された 32 ノード(実行中の
ノードとその祖先が優先され、残りはソースの順で補われます)しかないため、ツリー全体は
`node_id` なしの `agent.activity.read` で取得してください。
...
一致します)。`node_id` を指定するとレスポンスは `content` チャンクを含みます:`format`
(`text`、`markdown`、`jsonl`、`unknown`)、`text`、`eof`、`truncated`、そして次のページで
`cursor` として返す `next_cursor` です。`cursor` は呼び出し側にとって不透明です。
...
エラー:`pane_not_found`、`agent_not_found`(ペインにエージェントがいない、外部
ソースが未知、またはそのソースがその外部セッションをもう列挙していない)、`not_implemented`(このサーバーにそのエージェントのソースアダプターが
ない、またはアダプターがこのリクエストに応答できない。たとえば Herdr がまだその
エージェントのネイティブセッション参照を受け取っていない)、`activity_unavailable`(ソースを
今は読めない。たとえば Pi がまだスナップショットを報告していない)、`activity_malformed`
(ソースの内容またはカーソルが期待した形式ではない)、`activity_io_error`、読み取り
キューが満杯のときの `server_busy`。
...
`external_id` として使えます)、`source`、`agent_status`、`label`、`readable` と、任意の
...
ツリーは `agent.get` または `agent.activity.read` で読み取ってください。

...
Local と複数の SSH マシンを 1 つのウィンドウで使うには、[マシンへの接続](/ja/docs/connecting-machines/)を参照してください。セットアップ、切り替え、再接続、設定、リモート自動化を説明しています。保存済みマシンを削除してもクライアントが切断されるだけで、リモートセッションは停止しません。転送はあらゆる場面で素の SSH のままです。不安定なネットワークでも、常駐するリモートサーバーがセッション状態を保持し、クライアントがバックオフ付きで再接続するためです。これが Herdr が mosh を使わない理由でもあります（[マシンへの接続](/ja/docs/connecting-machines/)の「mosh を使わない理由」を参照）。
...
TUI 内では、ホスト鍵や認証が原因の Attention 失敗を、マシンの詳細カードからガイド付きで回復できます（[マシンの接続 — 接続の問題](/ja/docs/connecting-machines/) を参照）。提示された SHA256 フィンガープリントを確認してから記録する初回信頼ダイアログ、変更された鍵を強くブロックするダイアログ、そして明示的に承認された対話的認証経路があります。承認された対話では、SSH のプロンプトが Herdr 自身の askpass ヘルパー（`HERDR_SSH_ASKPASS_SOCKET` 環境変数で識別される再起動された `herdr` プロセス）を通じてマスク付き入力ダイアログに表示され、回答はプロセスメモリ内にだけ存在し、ログにも永続化にも残りません。承認がない限り、すべての接続はこれまでどおり完全に非対話です。

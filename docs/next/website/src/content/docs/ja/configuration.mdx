...
未設定のフィールドはセマンティックなフォールバックを保持します: ペインの境界線は `accent`/`overlay0`、スクロールバーのつまみは `overlay1`（フォーカス時）または `overlay0`、トラックは `overlay0`（フォーカス時）または `surface_dim`、トーストの境界線は `green`/`blue`/`red`、モードバーは `accent` を使います。`selection_mix_ratio`（0.0–1.0、デフォルト 0.28）は、選択背景をホストの背景色から白または黒へどの程度混ぜるかを制御します。選択されたテキストの太字・斜体・下線は保持され、セルの色だけが変わります。全フィールドの一覧は[設定リファレンス](/docs/config-reference/)で `theme.components` を検索してください。

...
サイドバーの行設定は、展開されたデスクトップサイドバーにだけ適用されます。折りたたみ表示とモバイル表示はコンパクトなレイアウトを保ちます。グループ表示のエージェントツリーでは、上のグループヘッダーがすでにマシン・ワークスペース・タブを示しているため、エージェント行から `machine`、`workspace`、`tab` トークンを省きます。その結果 `state_icon` だけが残った行は次の行と結合されます。どの行にも `state_text` がないときは、幅に収まればエージェント名の後ろに状態テキストを補います。
...
エージェント別のサウンド上書きには `default`、`on`、`off` を指定できます。キーは 5 つの公式エージェントのラベル `pi`、`claude`、`codex`、`open_code`、`kimi` です。[廃止されたエージェント](/ja/docs/integrations/#廃止されたインテグレーション)を指すキーは `unknown config key` の診断とともに無視され、セクションの残りの設定はそのまま適用されます。

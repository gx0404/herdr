...
Herdr のメインメニューを開いて `reload config` を選ぶこともできます。
...
`language` は Herdr 自身の TUI と CLI メッセージの言語を選びます。`"zh-CN"`(既定)と `"en"` に対応しています。

```toml
language = "en"
```

環境変数 `HERDR_LANG` を指定すると、その 1 回の実行だけ上書きできます。この設定を含むクライアント設定はライブリロードに対応しており、`reload config`(またはメインメニューの `reload config`)でペインを再起動せずに反映されます。
...
ドッキングしたターミナルグループでは、表示中のタブごとに独立したビューサイズを使います。従来の単一ビュークライアントでは、次の動作を維持します。クライアントが 1 つだけ接続されている場合、すべてのタブがそのサイズに従います。複数のクライアントが接続されている場合、表示中の各タブは、最後にそのタブへフォーカス、選択、または操作したクライアントのサイズに従います。1 つを除くすべてのクライアントが切断すると、すべてのタブは残ったクライアントのサイズへすぐに戻ります。クライアントが 1 つも残っていない場合、既存のペイン PTY は最後のサイズを保ち、新しいヘッドレスレイアウトには設定したフォールバックが使われます。
...
ペインは Herdr 自身の端末レイヤーで描画されるため、サーバーを起動した端末ではなく Herdr として名乗ります: `TERM_PROGRAM=herdr`、`TERM_PROGRAM_VERSION` は Herdr のバージョン、`TERM=xterm-256color`、`COLORTERM=truecolor`。ホスト端末の識別変数（`WEZTERM_PANE`、`WEZTERM_UNIX_SOCKET`、`WEZTERM_EXECUTABLE`、`WEZTERM_EXECUTABLE_DIR`、`WEZTERM_CONFIG_FILE`、`WEZTERM_CONFIG_DIR`、`KITTY_WINDOW_ID`、`WT_SESSION`）は取り除かれるため、ペイン内のプログラムが Herdr をホスト端末と誤認しません。`HERDR_ENV=1` は常に設定され、Herdr ペイン内で動くプロセスの目印になります。

Unix では、ペインを生成するたびに `SSH_AUTH_SOCK` が検証されます: パスが存在し、socket で、現在のユーザーが所有している必要があります。サーバーが継承した agent socket が無効になっている場合（ホスト端末の再起動後など）、Herdr は直近にアタッチされたフォアグラウンドクライアントがレポートした値にフォールバックし、どちらも使えないときは変数自体を設定しません。これにより、ペイン内の `ssh` は死んだ socket に張り付く代わりに「agent なし」の明確なエラーで失敗します。

シェルは `OSC 7`（`file://<host>/<path>`）で作業ディレクトリをレポートできます。`<host>` が空、`localhost`、またはマシン自身の hostname（大文字小文字を区別しない）の場合に Herdr はレポートを受け付けます。それ以外のホストを名指すレポートはリモートとして無視されます。受け付けられたディレクトリは `new_cwd = "follow"` を駆動し、サイドバーと `herdr pane list` にペインの cwd として表示されます。

Herdr はフォーカスされたペインの作業ディレクトリを `OSC 7`（`file://<host>/<percent エンコード済みパス>`）としてホスト端末にも返します。送信はフォーカスまたはディレクトリの変化時だけで、フレームごとには行いません。`OSC 7` を理解するホスト端末（WezTerm、kitty、foot など）はそのディレクトリで新しいタブや分割を開くため、`Ctrl+Shift+T` で開いた新しいホストタブはフォーカス中のペインの場所に落ちます。
...
worktree アクションは Git ワークスペースの行から使えます。`New worktree` はチェックアウトを作成し、入力したブランチがすでに存在する場合は既存のローカルブランチをチェックアウトし、存在しない場合はブランチを作成します。その後、新しい Herdr ワークスペースとして開き、元のワークスペースの下にグループ化します。`Open worktree...` はそのリポジトリにある既存の Git worktree チェックアウトを一覧表示します。すでに開いているチェックアウトを選ぶとそこへフォーカスし、閉じているチェックアウトを選ぶと同じグループで開きます。フォルダーが削除されても Git にまだ登録されているチェックアウトは `prunable` ラベル付きで一覧に残り、選ぶと開く代わりに `git worktree prune` の実行を促すヒントを表示します。ただし、そのチェックアウトがまだ Herdr ワークスペースとして開いている場合は、選ぶとそのワークスペースにフォーカスします。
...
worktree チェックアウトの削除は明示的に行います。グループ化された子ワークスペースで `Delete worktree checkout...` を使うと `git worktree remove` が実行されます。Herdr はまず Git に安全な削除を要求します。チェックアウトに変更済みまたは未追跡のファイルがあり Git が拒否した場合、Herdr は強制削除を実行する前にもう一度確認します。この 2 回目の確認は意図的に Enter ではありません。`y`（または `ctrl+↵`）を押すか、「y delete anyway」をクリックします。これにより前のステップでの Enter の長押しや二連打が強制削除まで進むことはありません。このステップで既定の強調ボタンはキャンセルです。ブランチは削除されません。
...
デフォルトのキーマップはプレフィックス優先なので、Herdr がシェル、エディタ、tmux、ターミナルアプリから入力を奪うことはありません。[設定リファレンス](/docs/config-reference/)で `keys.` を検索すると、すべてのアクションとデフォルトのバインドを確認できます。`prefix+?` で開くアプリ内ヘルプパネルには、現在有効なバインドが表示されます。プレフィックスモードが有効な間、which-key ポップアップが利用可能なプレフィックスバインドを一覧表示します（`ui.which_key = false` で無効化）。`prefix+u` でリンクヒントモードに入ります: ペインのビューポートに見える URL に 2 文字のマーカーが付き、そのマーカーを打つとリンクが開きます。
...
`terminal` テーマは Herdr の構造色を ANSI パレットに対応させます: 入力フィールド、キーキャップ、非フォーカスタブが使う弱い構造面は `DarkGray`、より強いホバー / アクティブ面は `Gray`、本文とパネル背景はターミナル自身の色のままです。そのため 16 色ターミナルでも入力フィールド、ホバーのハイライト、無効ボタンの境界が消えず、通常のテキストに溶け込みません。
...
`sidebar_bg` を使うと、デスクトップのサイドバーだけに背景色を設定できます。ドッキングレイアウトではワークスペース一覧と Agents パネルの両方にこの色が使われます。設定しない場合、既定の `catppuccin` テーマはパネル色でサイドバーを塗り、ほかの組み込みテーマはホストターミナルの背景を使います。`active_row_bg` は、区切り線やスクロールバートラックに影響を与えず、アクティブな Space とフォーカス中の Agent 行の背景色を変更します。`selection_bg` は、サイドバーの Navigate モードのカーソル行の背景色を変更します。
...
コンポーネントレベルのトークンは、解決済みパレットの上で個々の UI 要素を微調整します。未設定のフィールドは対応するセマンティックトークンにフォールバックします:
...
未設定のフィールドはセマンティックなフォールバックを保持します: ペインの境界線は `accent`/`overlay0`、スクロールバーのつまみは `overlay1`（フォーカス時）または `overlay0`、トラックは `overlay0`（フォーカス時）または `surface_dim`、トーストの境界線は `green`/`blue`/`red`、モードバーは `accent`、`hover_bg`（メニューやオーバーレイ一覧でポインタが乗っている行）はパネル背景とはっきり区別できる最初のサーフェス色を使います。多くのテーマでは `surface1` ですが、`surface1` が `panel_bg` とほぼ同じ明るいテーマでは自動的により強い色に下がります。`selection_mix_ratio`（0.0–1.0、デフォルト 0.28）は、選択背景をホストの背景色から白または黒へどの程度混ぜるかを制御します。選択されたテキストの太字・斜体・下線は保持され、セルの色だけが変わります。全フィールドの一覧は[設定リファレンス](/docs/config-reference/)で `theme.components` を検索してください。
...
`ui.border_style` はペイン境界線のグリフを `"single"`（デフォルト）、`"rounded"`、`"double"`、`"thick"` から選びます。

`ui.color_depth` は Herdr 自身の UI 色の出力色深度を制御します: `"auto"`（デフォルト）はホストターミナルに従い、truecolor の証拠がない `256color` 系の TERM（SSH や tmux で一般的）では、テーマの RGB 色が自動的に最も近い xterm-256 パレットエントリにマッピングされます。`"truecolor"` で 24 ビット出力を強制し、`"256"` でダウングレードを強制します。

マウス操作のタイミングは調整できます: `ui.double_click_ms`（デフォルト `350`）はダブルクリックの語選択・トリプルクリックの行選択に使う時間窓であり、スニペット一覧で同じスニペットを 2 回目にクリックして実行を開始する際の時間窓でもあり、`ui.drag_throttle_ms`（デフォルト `33`）はペインのスクロールバーと分割バーのドラッグ更新を間引き、`ui.selection_autoscroll_interval_ms` / `ui.selection_autoscroll_min_lines` / `ui.selection_autoscroll_max_lines`（デフォルト `30` / `3` / `15`）はペイン端を越えてドラッグしたときの選択範囲の自動スクロールを制御します。

入力フレーミングの待ち時間も調整できます: `ui.input_sequence_timeout_ms`（デフォルト `150`）は、読み取り境界で分割された不完全な制御シーケンス（キーエンコーディング、マウスレポート、裸の `ESC [` 導入部）の残りのバイトをクライアントが待つ時間で、超過するとその断片は破棄されます（そのため本物の Alt+[ もこの窓の経過後に送信されます）。`ui.escape_after_mouse_timeout_ms`（デフォルト `30`）は、マウスキャプチャが有効な場合（または直近 0.5 秒以内にマウスレポートを受信していた場合）に、単独の Escape がマウスレポートの続きを待つ時間です。それ以外では Escape は 10 ms のアイドル窓の後に送信されます。遅延の大きい SSH 接続では両方を大きくしてください。どちらも `10..=1000` に丸められ、クライアント起動時に読み込まれ、Unix クライアントのみに適用されます（Windows クライアントは固定の 10 ms でポーリングします）。
...
サイドバーの行設定は、展開されたデスクトップサイドバーにだけ適用されます。折りたたみ表示とモバイル表示はコンパクトなレイアウトを保ちます。グループ表示のエージェントツリーでは、上のグループヘッダーがすでに示しているマシン・ワークスペース・タブについては、エージェント行から `machine`、`workspace`、`tab` トークンを省きます。その結果 `state_icon` だけが残った行は次の行と結合されます。どの行にも `state_text` がないときは、幅に収まればエージェント名の後ろに状態テキストを補います。
...
アプリ内トーストには `herdr`、SSH 越しでも使いやすい外側のターミナル通知には `terminal`、ローカル OS の通知サービスには `system`、ポップアップを無効にするには `off` を選びます。Herdr はアクティブなタブのポップアップを抑制します。位置、遅延の挙動、クリップボードのフィードバック設定については、[設定リファレンス](/docs/config-reference/)で `ui.toast` を検索してください。 通知バナーとトーストには発信元ペインの表示名（無い場合は pane id）が付くので、モバイル幅でも出所が分かります。
...
エージェント別のサウンド上書きには `default`、`on`、`off` を指定できます。キーは公式エージェントごとに 1 つで、`pi`、`claude`、`codex`、`open_code`（OpenCode。アンダースコア付きです）、`kimi` です。[廃止されたエージェント](/ja/docs/integrations/#廃止されたインテグレーション)を指すキーは `unknown config key` の診断とともに無視され、セクションの残りの設定はそのまま適用されます。
...
ハードウェアカーソルを隠す AI Agent TUI は、ホスト端末からカーソルアンカーを奪います。その結果、変換候補ウィンドウがフォーカス中のペインを追跡しなくなり、入力キャレットも途中で見えなくなります。Herdr は既定で点滅バーのアンカーを表示します。無効化や範囲の絞り込みは次で行います:

```toml
[experimental]
reveal_hidden_cursor_for_cjk_ime = true # 既定で有効
cjk_ime_cursor_shape = "bar"            # 既定は点滅バー
cjk_ime_agents = []                     # 空欄 = 任意のフォーカスペイン
```
...
アプリケーションが同期出力（`DECSET 2026`）で再描画を囲む場合、バッチが開いている間はカーソルを隠さず直前の状態を保つため、ホストはフレームごとに隠す/表示の組を受け取りません。1 秒を超えて閉じられないバッチは失効と見なされ、Herdr は描画を再開してカーソルは再びペインに追従します。モードを戻さない TUI がホスト端末からカーソルを奪うことはありません。

Herdr 自身の各フレームも同期出力バッチで囲まれ、既定ではバッチ終了後に最終カーソルアンカーをもう一度送り、`DECSET 2026` 内のカーソル移動を無視するネイティブ IME でもキャレットを追跡できるようにします。この追加の素のカーソル移動が見えてしまう端末では、それはカーソルやタブ切り替え時のちらつきとしてしか現れないため、`ui.repeat_ime_cursor_anchor` はクライアント起動時に一度だけ判定されます:
...
`auto` は `TERM_PROGRAM`、`TERM` または `KITTY_WINDOW_ID` が、再送がちらつきとして見えることが分かっている端末（WezTerm、kitty、ghostty、foot、contour、iTerm2、Alacritty、rio）を示す場合に再送を省略します。不明なホスト、および tmux や screen の中では再送を維持します。マルチプレクサは外側の端末の識別情報を引き継ぎますが、同期出力のパススルー挙動はバージョンによって異なるためです。判定は端末名のみでバージョンは見ません。これらの端末の古いリリースで IME がキャレットを見失う場合は `always` を、不明なホストがすでに同期出力をアトミックに描画する場合は `never` を設定して再送を止めます。Windows ではこの設定に関係なく再送しません。この設定はクライアント自身のシェルが描画するフレームにのみ影響し、`herdr terminal attach` セッションはサーバー側で描画されるため既定の再送を維持します。
...
| `GIT_CEILING_DIRECTORIES` | Git と同じ意味で尊重される。ワークスペースのラベル、ブランチトークン、worktree アクションに使うリポジトリ探索は、列挙したディレクトリの手前で止まる。 |
...
ログは自動的にローテーションされます。1 ファイルの上限は 5 MB で、直近 2 世代のローテーション済みコピーが同じディレクトリに保持されます（`herdr-server.log.1`、`herdr-server.log.2` など）。問題を報告するときは、現在のログとローテーションされた関連ファイルを含めてください。

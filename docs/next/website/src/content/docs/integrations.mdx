...
- Hooks or plugins that an earlier build installed into a retired agent's config directory are left untouched. Remove them by hand if you no longer want them; their session reports are treated as unofficial and are not used for restore.

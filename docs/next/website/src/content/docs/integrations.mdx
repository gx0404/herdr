...
A Codex sub-agent is labeled with its nickname and its task name, which is the last part of its agent path, for example `Boyle · herdr_probe`; the full agent path is its summary. A sub-agent without a nickname shows just the task name.

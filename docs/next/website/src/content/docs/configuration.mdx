...
Mouse interaction timings are tunable: `ui.double_click_ms` (default `350`) is the window in which repeated pane clicks become double-click word selection or triple-click line selection, `ui.drag_throttle_ms` (default `33`) throttles pane scrollbar and split divider drag updates, and `ui.selection_autoscroll_interval_ms` / `ui.selection_autoscroll_min_lines` / `ui.selection_autoscroll_max_lines` (defaults `30` / `3` / `15`) control selection auto-scroll while dragging past a pane edge.

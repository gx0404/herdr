...
Right-click menus draw separators between groups, show each entry's current shortcut on the right, dim entries that do not apply right now, and mark on/off entries with a check. From the keyboard, the arrow keys skip dimmed entries, typing an entry's first letter jumps to it (press it again to cycle through entries with the same letter), `→` / `←` open and close a submenu such as the pane menu's **View**, and `Esc` closes an open submenu before the menu itself. Because letters always jump by first letter, `j` / `k` / `g` / `G` no longer move the highlight as they did in earlier versions; use the arrow keys and `Home` / `End` instead. Right-clicking a machine in the sidebar lists the same per-machine actions as the main menu's machines category, including **Switch to this machine**.

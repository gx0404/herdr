...
/// 前台进程组观测（每 tick 的廉价探测）；瞬态默认色 owner 用当前前台 pgid
/// 纯计算。整条链不读 /proc。（原 droid CSI 3J 兼容判定随上游 309749ad 删除。）
fn maintain_foreground_job_cache(terminal: &PaneTerminal, pid: u32, foreground_pgid: Option<u32>) {
    terminal.note_foreground_pgid_observation(foreground_pgid);
...
fn spawn_basic_detection_task(
    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            ...
            maintain_foreground_job_cache(&terminal, pid, foreground_pgid);

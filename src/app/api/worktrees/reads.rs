        ...
        let response = match result.result {
            ...
            Ok(data) => {
                ...
                match result.request.method {
                    ...
                    // APP-012：不做 expect；发现阶段没选出条目按未找到返回。
                    Method::WorktreeOpen(params) => match data.entries.into_iter().next() {
                        Some(entry) => {
                            self.finish_worktree_open(result.request.id, params, source, entry)
                        }
                        None => encode_error(
                            result.request.id,
                            "worktree_not_found",

...
fn agent_status_event(index: usize) -> EventEnvelope {
    EventEnvelope {
...
fn assert_subscription_history_loss(agent_status: bool) {
    ...
    // fork 的断层判据按订阅种别：只有本订阅关心的种别被挤出保留窗口才算丢失。
    for index in 0..600 {
        test.hub.push(if agent_status {
            agent_status_event(index)
        } else {
            renamed_event(index)
        });
    }

    ...
    pub(in crate::client::shell) fn compose_workbench(
        ...
        for (panel, area) in &self.workbench.geometry.panels {
            ...
            if let PanelId::Terminal(id) = panel {
                let add = Rect::new(
                    area.right().saturating_sub(6).max(area.x),
                    area.y,
                    3.min(area.width),
                    1,
                );
                put(&mut buffer, add, " + ", Style::default().fg(palette.accent));
                self.workbench.hits.push((add, Action::NewTab(*id)));
                let Some(group) = self
                    .workbench
                    .dock
                    .groups
                ...
                *scroll = (*scroll).min(group.tabs.len().saturating_sub(1));
                for tab_id in group.tabs.iter().skip(*scroll) {
                    ...
                    let rect = Rect::new(x, area.y + 1, width, u16::from(area.height > 1));
                    if width == 0 {
                        break;
                    }
                    let selected = group.active.as_ref() == Some(tab_id);
                    put(
                        &mut buffer,
                        rect,
                        &label,
                    ...
                    self.hits.tabs.push((rect, tab_id.clone()));
                    x = x.saturating_add(width).saturating_add(1);
                }
            }
        }
        ...
        if let Some(mut composed) = frame.to_ratatui_buffer() {
            for hit in &self.hits.panes {
                ...
                if let Some(selection) = self
                    .selection
                    .as_ref()
                    .filter(|selection| selection.pane_id == hit.pane_id)
                {
            ...
            if let Some(notice) = &self.visible_endpoint_notice {
                occlusion.cover(super::super::endpoint_notices::render_notice(
                    &mut composed,
                    full,
                    notice,
                    1,
            ...
            if let Some(notification) = &self.visible_notification {
                self.hits.notification_toast =
                    super::super::notifications::render_visible_notification(
                        &mut composed,
                        full,
                        notification,
